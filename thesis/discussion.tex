...
\todo[inline]{below, try to make one paragraph per topic with a) definition, b)
example from current thesis, c) pro, con; then, delete subsections' titles}

\todo[inline]{finalize the merge of open-source software \& custom code into
"acquiring open data" \& "providing open data" respectively}
...
\todo[inline]{give one sentence def of open data (incl. preprocessed data \&
results)}



\paragraph{Example 1: Open data}
...
\todo[inline]{A sentence about pro of open-source is missing}
...
\todo[inline]{In summary: a study on the same dataset would have to mimic the
(non-automatized) procedure of \citep{sengupta2016extension}}
...
\todo[inline]{Try to merge "providing code" into here to create a "providing a
dataset"}
...
\todo[inline]{Pro here \& following Cons have probably big intersection of text
that can be dropped}

\todo[inline]{generally: phrasing!}

\todo[inline]{focus on immediate benefits}
% better organized, better documented
% hypothesizing after the results are known \citep[a.k.a. ``HARKing''; cf.][]{kerr1998harking}
...
\todo[inline]{shorten following paragraph}
%
...
\todo[inline]{learning via reading or "trying it yourself"; better understanding
(than from a method section); essentially, a step-by-step guide}
