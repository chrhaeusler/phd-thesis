...
\todo[inline]{This discussion is about the naturalistic stimuli in general; it
might be possible to merge it in to the discussion above}

\todo[inline]{Keep this part a little longer (same idea differently phrased),
because some stuff will be restated in the general discussion)}
...
%
It requires the researcher to model a hypothesized hemodynamic time course that
is fit to the data in order to predict and contrast the observed hemodynamic
activity \citep{friston1998event}.
%
...
perceptual difference of consecutive movie frames, and or low-level auditory
features like root-mean square power (a.k.a. volume) or left-right volume
difference can be extracted automatically reliably at [on?] a low temporal scale
...
%
Current advances in machine learning allow the creation tools (e.g., the
software package 'pliers' [McNamara et al., 2017]) that can automatically
extract higher-level features like clearly defined object categories or
semantics in order to substitute tedious manual annotation, or at least provide
a provisional scaffold that alleviates manual labor \citep[cf. the toolbox
``pliers'' that is implemented in the
\href{https://neuroscout.org/}{\url{neuroscout.org}}
platform;][]{mcnamara2017developing, delavega2022neuroscout}.
%
However, subject-related variables \citep[e.g., felt
emotions,][]{lettieri2019emotionotopy}, subjective and ambiguous, or variables
that fluctuate at [on?] longer time scales (e.g., context) still defy an
...
time-consuming\todo{nobody does it anyway}.
%
Further, depending on the covariance of variables, a naturalistic stimulus might
eventually turn out be inappropriate.
%
Last, a large amount of annotated features might lead to annotations that are
...
\todo[inline]{These literal quotes might 100\% match the point Richard tried to
make (his phrasing is ambiguous anyway)}
...
\todo[inline]{despited being designed to be engaging, the lack of task leaves
room for more individual variation that is difficult to record; especially
shitty when individual variation is considered noise}
...
%
In case of model-driven analyses, easing the``annotation bottleneck''
\citep[][p. 16]{aliko2020naturalistic}  of naturalistic stimuli is probably the
biggest prerequisite to test physiological assumptions, tackle methodological
issues, and develop appropriate analyses [cf. Nunez-Elizalde (2019), Dupre la
Tour (2022) for recent examples] in order to draw valid inferences from naturalistic stimuli.
