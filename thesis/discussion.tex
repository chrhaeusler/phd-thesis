\todo[inline]{Print intro and phrase parts here accordingly}

\section{Theory part}

\todo[inline]{This is supposed to be an updated version of the text in 1.1 (functional localization) to 1.3.3 (Application 2: Estimation from ref)}
...
\section{Aims of thesis \& overview}

\todo[inline]{cf. 1.3}

\todo[inline]{Don't get into details here, esp. regarding results; this will be
done after the open-science-excurse}


\subsection{Actual goals: naturalistic stimuli as functiona localizer?}
...
\paragraph{\citet{haeusler2021speechanno}}

\todo[inline]{if not already mentioned in the theory part above, mention here
that this is considered to be "prohibitively difficult"}

%
First, we lay the non-technical [???] groundwork.
%
En passant, this is also an extension of the studyforrest dataset.


\paragraph{\citet{haeusler2022processing}}
%
...
\section{Discussion across studies}

\todo[inline]{mih: aggregate across studies; show how things are connected}

\todo[inline]{cf. 1.3 \& 1.3.5}

\todo[inline]{This might be merged into "Naturalistic stimuli as functional
localizer?" below}

\todo[inline]{for now: structure, clean, shorten this part}
...
%
Second, we observed higher intersubject variability  of responses of the
\ac{ppa} to a naturalistic auditory stimulation [because the reliability across
segments sucks] compared to the visual stimulation during the localizer and
movie paradigm.
...
\paragraph{Reliability of audio-description as localizer}
...
discussion (it's mentioned in the paper, and discussed in "open science"
regarding opportunity costs)}

%
Our results invite further studies that investigate the properties of the
parahippocampal area in response to
%
a) in response to a controlled paradigm (with or without any task).
%
b) in response to an auditory naturalistic stimulation (with or without a
simultaneous task to attend to spatial information, or with subsequent memory
task),
%
The respective paradigms could elaborate whether
%
a) anterior PPA is also the case in controlled paradigms
%
b) reliability can be increased by sampling more events in less time (fatigue),
improve modeling, control for alertness.
...
\citet{haeusler2022processing} suggest:
...
individual subjects.
%
But results are far from perfect and won't substitute the localizer.


%
Still, modeling hemodynamic responses to annotated stimulus features detected
responses correlated to auditory spatial information (in the PPA).
%
Hence, the audio-description samples response vectors that might be consistent
across subjects and "catchable" for a \ac{srm}.
%
We might be able to use the response patterns measured during the
...
If the response vectors sampled during auditory naturalistic stimulation are
similar to the ones that are correlated pics of landscapes in the visual
localizer we can predict the localizer \todo{if you know what I mean}
...
We estimated results of a dedicated localizer \citep{sengupta2016extension}
via functional alignment from results of a
reference group.
...
Nevertheless, the primary movie contrast in \citet{haeusler2022processing}
``yielded bilateral clusters in five participants, a unilateral right cluster in
six participants (of which one participant yielded a unilateral cluster in the
visual localizer), and a unilateral left cluster in one participant. We find
bilateral clusters for participant sub-20, whereas the block-design localizer
...
\subsection{However, for now, better stick to localizer as the established
method}
...
\todo[inline]{There is no functional localizer based on naturalistic stimuli
for a reason yet! Probably, for a reason...}
...
%
What does the PPA actually do? What is the psychological construct?
%
Yet, we estimate the $Z$-map of a visual localizer which is the fucking
established / traditional method to identify the \ac{ppa}.
%
It's reliable (in most block-design paradigms at least?) and yield bilateral
clusters of increased hemodynamic activity in most subjects.


Template from \citet{jiahui2020predicting}:
%
``We found more reliable estimates
...
2014; Thorat et al., 2019],
%
biological motion [Beauchamp et al., 2003; Bonda et al., 1996; Çukur et al.,
2013; Gobbini et al., 2007; Grossman and Blake, 2002],
%
social interaction [Castelli et al., 2002; Çukur et al., 2013; Gobbini et al.,
2007; Schultz et al., 2003],
%
and agentic behavior [Gobbini et al., 2011; Nastase et al., 2017],
%
suggesting that the ground truth for the functional selectivity of the system
identified by face-selectivity may extend well beyond face-selectivity per se.
%
...
Template from \citet{jiahui2022cross}:
%
...
\section{But functional alignment rocks: scan once, estimate many!}
...
Templates from \citet{jiahui2020predicting}' discussion:
%
...
\section{General conclusion: naturalistic stimuli are no panacea}
