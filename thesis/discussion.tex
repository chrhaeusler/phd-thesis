...
\todo[inline]{If SRM part will be dropped, just merge with the part above}

\todo[inline]{If SRM part is supposed to stay: final points to make, their
location, and exact phrasing depend on where which parts of "Naturalistic Stmuli
for Functional Alignment" will be discussed how extensively (cf. "The Vision:
Scan once, estimate many")}


\subsubsection{Origin and assumptions of GLM}
%
...
statistical power due to insufficient data samples.\todo{correct?}

\todo[inline]{These literal quotes might not 100\% match the point Richard tried
to make (his phrasing is ambiguous anyway)}
...
\todo[inline]{despite being designed to be engaging, the lack of task leaves
...
\todo[inline]{Add physiological recordings, combine with EEG, eye-tracking, add
self-reports [assess controll alertness, engagement, audibility etc.]}
