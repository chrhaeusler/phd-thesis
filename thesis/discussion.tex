...
participants.
%
To advance the field towards a clinical application, data need to be assessed on
...
a visual localizer by modifying the procedure for the analysis of data from a
standard localizer paradigm to data from the two naturalistic paradigms,
%
...
%
This objective included to using open data and open-source software, and
...
open-source software to prevent creating a ``artificial paywall'' for running
...
This is particularly important as the standards (quality, formats, parameters)
and open sciences practices (e.g., documenting) may differ across scientific
fields or even within a scientific field depending on a working group's
expertise and rigor.
...
pre-existing data or to preprocess the data themselves.
%
...
activation in the \ac{ppa} during the perception of static pictures generalizes
to the perception of spatial information embedded in a movie and an exclusively
auditory stimulus.
%
...
%
The currently dominant analysis approach is the General Linear Model (GLM),
which has its roots in \ac{pet} research, and is tailored to analyze data of
parametric experimental designs that manipulate isolated experimental variables
of interest.
%
The GLM requires the researcher to specify which stimulus features are presumed
to be correlated with the brain process under investigation.
%
...
the traditional GLM approach, such as cognitive subtraction [Friston, 1996], the
consistency of hemodynamic responses across events \citep[the rationale behind
\textit{trial-averaging};][]{dale1997selective}, and the linearity of
hemodynamic responses (Cohen, 1997; Boynton, 1996; Dale, 1999).  \todo{if points
are correct, extend}
%
...
%
Although naturalistic stimuli are intended to be inherently
engaging and offer a task-free paradigm, sustained attention is still required
by participants when ``freely viewing'' a movie or ``freely listening'' to an
auditory narrative, which may become challenging over longer periods of time.
%
...
%
Nonetheless, a varying level of engagement might reflect a personal preference
...
\todo[inline]{This is too long; shift stuff that is not mentioned in the
discussion (enough) into the discussion and merely stick to recommendations
here?}
...
% considerations before using a naturalistic stimulus at all
Researchers should carefully consider the strengths and limitations of
...
Annotating and sharing naturalistic stimuli is crucial to overcoming the
...
% naturalistic stimuli are not supposed to replace traditional paradigms
% entirely
