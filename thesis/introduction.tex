...
\todo[inline]{where could auditory narrative / auditory PPA primed (more)?}
...
The lack of extensive annotations has led to a ``usage bottleneck'' \citep[][p.
16]{aliko2020naturalistic} and might be the main reason why explicit models of
embedded stimulus features are ``notoriously'' \citep[][p. 1]{richard2019fast},
if not ``prohibitively'' \citep[p. 676]{nastase2019measuring} difficult to
construct.
...
\todo[inline]{I / my vs. We / our? Maybe, just switch to passive form}
...
\todo[inline]{It's not written in the theory part above already (because it
get's super messy there); therefor, the part here can be a little longer}
...
so vague that it gets almost wrong; at least I would be puzzled more if the text
would give less details}

\todo[inline]{"transformation matrix" was substituted with "mapping"}
...
%
One goal of the disseration was to meet both the requirements of open, shared,
...
A second goal in the context of open science reuse a publicly and freely
available dataset for a new reasearch question that was not anticipated during
the data's initial release, extend the dataset, and generate novel findings to
be published in open-acess journals.

%
The present work is built upon \ac{fmri} data that are part of the
\textit{studyforrest} project
(\href{www.studyforrest.org}{\url{studyforrest.org}}), an open science project
...
%
In order to lay the groundwork for the presently performed analyses and to
further extent the studyforrest dataset as a versatile resource for independent
researchers an additional annotation of speech occuring in the two stimuli was
created and published \citep{haeusler2021speechanno} in a standardized file
format.
%
...
datasets.
% automatization
Therefore, data analysis pipelines are additionally designed in a way that
enables automated processing.
% FOSS
Further, analyses pipelines are not implemented in proprietary software but in
freely available and, if possible, open-source software.
...
\todo[inline]{details on results are reduced; imo, reporting some results make
...
of its user interface and documentation, and by giving feedback to the team of
software engineers.
%
This resulted in a co-authorship of the software's accompanying publication
\citep[cf.][]{halchenko2021datalad}, and laid the technical foundation that
enabled archieving the goal of conducting a transparent and reproducible
research project.
...
\todo[inline]{Following results reported too detailed?}
...
\todo[inline]{proof of concept without "innovative" analysis / method}
