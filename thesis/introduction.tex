...
10]{savoy2001history}.
% fMRI + BOLD
Since the early 1990s, brain imaging studies have used \ac{fmri} to measure
\ac{bold} activity as a proxy for neural activation.
% higher visual areas
Replicated findings in the domain of high-visual perception, for example,
suggest that category-selective brain regions, such as the parahippocampal place
...
increased \ac{bold} activity correlated with a ``preferred'' \citep[][p.
123]{debeck2008interpreting} stimulus class.
%
The \ac{ppa} responds more strongly when participants are viewing images of
landscapes or landmarks compared to images of, e.g., faces;
%
vice versa, the \ac{ffa} responds more strongly when participants are viewing
...
individuals \citep{dubois2016building, wegrzyn2018thought}.
...
However localizers are designed to maximize detection power, use carefully
selected, simplified and tightly controlled stimuli presented in a block-wise
manner, and are often accompanied by a task to keep study participants
attentive.
...
given by the experimenter, and place minimal task demands on study
...
stimulation that can put participants at ease in the uncomfortable and noisy MRI
scanner.
%
...
\section{Ethics protocol}

All data acquisitions were jointly approved by the Ethics Committee of
Otto-von-Guericke- University of Magdeburg, Germany (approval reference 37/13).
...
% main goals
This dissertation aimed to explore---while adhering to the principles of open,
...
To the author's knowledge, only one study \citep{aziz2008modulation} compared
...
the \ac{ppa} does not solely respond to visually presented scene-related
stimuli.

% SRM
The thesis assessed the feasibility of using the movie and audio-description as
alternatives to a visual localizer in two ways.
% 1. modeling hemodynamic responses
As the first approach, we modeled hemodynamic activity based on annotated
...
\citep{frost2012measuring, rosenke2021probabilistic, weiner2018defining,
zhen2017quantifying} estimated the location of a participant's functional area
by performing an \textit{anatomical alignment} that relies on anatomical scans.
%
...
functional alignment and subsequent estimation performance.
...
%
The first objective in the context of open science was to meet the requirements
of open, shared, accessible, and transparent science
\citep[cf.][]{watson2015will, fecher2014open} as well as a reproducible and
replicable research project:
%
...
The second objective in the context of open science was to reuse a publicly and
...
Bash), version-controlled, documented, and publicly released.
% inference
...
% neurovault Unthresholded statistical maps of all computed statistical
% $t$-contrasts are published at Neurovault
...
% results
This work laid the technical foundation for conducting a transparent
...
speech spoken in the movie, as well as its audio-description, with two
objectives in mind.
% goal #1: groundwork for PPA study
The first objective was to establish the groundwork for creating models of
hemodynamic activity in response to the movie and the audio-description in
\citet{haeusler2022processing}.
% goal #2: extend studyforrest
The second objective was to create an exhaustive annotation of speech in order
to extend the studyforrest project and supplement the previously published
...
annotation.
% contrast
Regressors correlating with speech-related events were contrasted with a
regressor correlating with events without speech.
% hypothesis
We hypothesized that results would reveal clusters of increased
...
In \citet{haeusler2022processing}, our objective was to investigate the
possibility of localizing the \ac{ppa} using GLM $t$-contrasts based annotations
of the naturalistic stimuli.
...
%
These results imply that a model-driven \ac{glm} analysis based on a
naturalistic stimulus' annotation can be used to localize functional areas in
individuals.
...
group from their respective voxel space through a \ac{cfs} into the
individual's voxel space.
%
...
data of each paradigm used for aligning a test subject with the \ac{cfs} and
subsequent estimation performance.
% Results
Results suggest that an auditory narrative can in principle be used to estimate
a visual area such as the \ac{ppa}.
%
Moreover, 15 minutes of functional scanning during movie watching can generate a
sufficient amount of data to estimate brain patterns with higher fidelity than a
