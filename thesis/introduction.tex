...
population-level inferences, on the flip side, may just ``capture the common
...
Currently, functional localizers are often used as an separate experiment to
...
to neurosurgery \citep[cf.][]{wegrzyn2018thought}.\todo{beinahe ein
Sekundärzitat}
...
scanner.
%
Therefore, naturalistic stimuli promise increased data quality due to
decreased fatigue and head movement, especially in the case of children
\citep{vanderwal2015inscapes}, and possibly psychiatric
\citep{eickhoff2020towards} or elderly persons.

\todo[inline]{use of audiobook will be first mentioned in "aims of thesis" (1.3)}
...
\todo[inline]{write the following in past, present or future tense?}
...
\todo[inline]{If study 4 is dropped, drop the following (until open science)}
...
\todo[inline]{first sentence is oversimplified; revise it}
...
The first aim was to build the groundwork that enabled us to create the \ac{glm}
contrasts based on annotated stimulus features in order to localize the \ac{ppa}
in study 2.
% aim #2: extend studyforrest
The second goal substantially exceeds goal one of creating the groundwork
necessary to conduct study 2:
%
the second aim was to extend the studyforrest dataset as a public resource for
independent research by creating an exhaustive annotation of speech the
...
\todo[inline]{Following part is a draft \& similar to the part written in
"overview" (1.3.1)}
