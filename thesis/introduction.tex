...
\todo[inline]{first, a little interim summary of intro so far}
...
% contra localizers
However, traditional localizer paradigms can usually map just one domain of
brain functions, employ selectively sampled, tightly-controlled stimuli, and
rely heavily on a participant's compliance.
...
\todo[inline]{use of audio-description mentioned here for the first time}
...
\paragraph{Procedure: anatomical alignment}
% previous studies: anatomical alignment
Previous studies have shown that the location of a participant's \ac{ppa} can be
estimated using anatomical alignment [e.g. CITATION]:
%
participants functional data are aligned (i.e. projected into) to a
\ac{cas} (e.g. MNI or Talairach brain template).
%
Then, results are projected from the \ac{cas} into the brain anatomy
...
\paragraph{Procedure: functional alignment}

\todo[inline]{this is supposed to be a short overview!}

\todo[inline]{maybe, a little more in specific aims}

Therefore following leave-one-subject out cross-validation, we compare an
estimation based on an anatomical alignment to a new procedure that is based on
a functional alignment.
% cfs creation
For every participant, we first create a \ac{cfs} that is based on the other
participants' response time series of the movie and audio-description.
% transformation matrices
A process that also computed transformation matrices for the subjects that
provided the data for the creation of the model space.
% projection of data
Data (statistical $Z$-maps) of the train subjects are projected into the
\ac{cfs}.
% alignment of left-out
We aligned left-out subjects with the \ac{cfs} to derive transformation matrices
for the left-out-subjects to perform the alignment.
% prediction
...
\paragraph{(partial) functional alignment}
% functional alignment
Here, we employ functional alignment that has been shown to outperform
anatomical alignment \citep{haxby2020hyperalignment, bazeille2021empirical}.
%
Given that, a two-hour paradigm would be inefficient plus unsuitable for a
clinical population, we explore whether just a part of a movie or auditory
narrative can serve as a ``diagnostic'' run in order to estimate the location of
the \ac{ppa} in an ``unknown'' subject's brain.
%
Therefore, we also assess the quantity of data needed to reliably predict
individual localizer results by testing which amount of data is needed to
perform an alignment to the \ac{cfs} that provides transformation matrices that
...
visual areas.
%
However considering practical and monetary constraints in a clinical context, 90
to 120 minutes are inappropriate for even an extensive individual diagnostic
procedure c

\paragraph{Solution}

%
Therefore in study 3,  we use data of the movie and audio-description to
predict localizer results \citep{sengupta2016extension}.
%
In study 3, we assess a procedure to estimate results of a dedicated localizer
based on data acquired during naturalistic stimulation.
...
\todo[inline]{training \& testing data is okay; make clear that we do not just
estimate parameters, i.e.  statistical inference but ``prediction''}
