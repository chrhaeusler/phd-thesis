...
increased validity and or generalizability for functional alignment over
transformation matrices based on data of a controlled paradigm based on
simplified stimuli.
...
%
Even one segment of audio-description is not statistically different from four
runs of localizer to estimate the localizer (after Bonferroni correction;
p=0.03).


\todo[inline]{cf. templates of \citet{haxby2011common} at the very end of this
chapter}
...
%
Our results are different results of \citet{haxby2011common} who found that
...
that have a poor Cronbach's; i.e. they are not "reliable outliers" being
...
\todo[inline]{Operationalization \& construct validity}
...
%
Hence, we should have modeled the construct "correctly" for >50\% of
...
%
At least for most subject that does not seem to be the case because it works.
...
samples the responses to (at least auditory) spatial information (in most
subjects at least).
%
...
%
Shared responses correlate with specific regressors, whereas shared features
during \acp{tr} of the movie / audio-description seem to be more abstract (or
the regressors simply suck).
%
Finally, the within-paradigm prediction works for most subjects.
%
...
\todo[inline]{define "reference database" better}
...
%
The \ac{cfs} is part of a \textit{reference database}.


\subsubsection{Reference database (incl. dedicated paradigms)}
...
\subsubsection{Align to database via diagnostic run}
%
...
[cf. the point in \citep{haeusler2022processing} about controlled paradigm +
task].
...
\subsubsection{Application: estimate regular pattern \& quantify (a reliable)
deviant pattern}
...
\paragraph{Example: \citet{yates2021emergence}}
...
\todo[inline]{better discuss in general discussion that naturalistic stimuli
...
\todo[inline]{Avoid a big point in the discussion by stating in the intro that
response based aligns responses better, connectivity-based aligns connectivity
better}
...
``RHA was shown to align response- based data better than CHA, whereas CHA was
shown to better align connectivity-based data than RHA''
...
%
However, it has remained unclear whether optimizations of both response
hyperalignment and connectivity hyperalignment would converge on the same common
information space.
%
While both response- and connectivity-based hyperalignment significantly improve
intersubject correlations (ISCs) of response profiles relative to anatomical
alignment, response-based hyperalignment (RHA) results in slightly higher ISCs
for response profiles than does connectivity-based hyperalignment (CHA)
(Guntupalli et al., 2018).
%
Similarly, RHA yields better alignment of cortical response patterns for two
additional tests of between-subject alignment: between-subject multivariate
pattern classification (bsMVPC) and ISC of representational geometry (Guntupalli
et al., 2016, 2018).
%
At the same time, CHA yields higher ISCs of dense connectivity profiles than RHA
...
vertices. Connectivity-based hyperalignment (CHA) mapped data from the
