...
%
We aimed to estimate the results of $t$-contrasts (i.e. $Z$-maps), that were
...
%
We aimed to estimate the empirical $Z$-maps that we created in previous studies
\citep{sengupta2016extension, haeusler2022processing} in order to localize the
\ac{ppa} serving as the three to predicted criteria
%
...
\todo[inline]{if stated at all, compress to 1-2 sentences}
...
First, we estimated the results of the visual localizer, which is the
established method for identifying the \ac{ppa}.
%
...
gap between estimation performance using the audio-description and the same
...
\todo[inline]{which leads to what exactly???}
...
located in the union of voxels that showed an significantly increased activity
...
%
When estimating the results of the visual localizer, the movie showed lower
prediction performance than the within-prediction.


\subsubsection{But movies are awesome}
...
social interactions, background music etc., we predict functional alignment
based on movies also will work for localizers of functional topographies for
audition, language, and social cognition'' \citep{jiahui2020predicting}.
%
Therefore, it may have better generalization capabilities across multiple
paradigms compared to the visual localizer.
...
\subsubsection{Therefore: future studies on other areas}
...
\paragraph{In anatomical alignment it varies, too}
...
between-subject variability as found in language areas
\citep{frost2012measuring}.
...
\todo[inline]{compare how good it works in other areas compared to anatomical
alignment}

\todo[inline]{how far can you push it (at what "costs")? Executive functions?}
...
%
We showed that we can estimate a visual paradigm from a auditory one but what
about other (processes that might not be sampled at all)
