...
\todo[inline]{revise title; write abstract}
...
\todo[inline]{delete (all?) subsections?}

\todo[inline]{present vs. past tense?}
...
$Z$-maps)---created in previous studies \citep{sengupta2016extension,
...
%
Each test subject's response time series to one of the paradigms were separately
used to functionally aligning the test subject with the corresponding paradigm's
...
%
Considering the challenges of acquiring functional data from a lengthy two-hour
paradigm to functionally an individual with a \ac{cfs}, we also explored the
...
sentences like in the abstract}

\todo[inline]{otherwise, following paragraph is a template for the conclusions
and/or abstract}
...
%
They found that the cross-subject classification of the traditional paradigm's
stimulus categories via the \ac{cfs} and transformations calculated from the
same paradigm's data outperformed the classification based on the movie data.
%
\citet{haxby2011common} interpreted these results gained from two paradigms
providing roughly the same amount of \acp{tr} as the controlled paradigm
sampling the investigated brain states more extensively than the movie.
%
...
approaches a performance limit, with diminishing benefits of longer scanning
time (cf. Fig.~\ref{fig:stripplot}).

%
We also explored a cross-paradigm prediction by using an auditory narrative
without visual stimulation to predict the results of the visual localizer.
%
...
audio-description improved and closed the gap with the prediction based on the
same amount of movie data.
%
...
audio-description did not outperform the prediction via anatomical alignment.
%
...
shared responses of lower peak amplitudes compared to the the movie and visual
localizer despite normalizing all time series to a mean of 0 and variance of 1
separately before concatenating the time series and caluclating the shared
responses.
%
The audio-description may have a lower signal-to-noise ratio\todo{is this the
case???}, or evoke responses that are less congruent across subjects resulting
...
\todo[inline]{if that makes sense at all, add plot of TRs of top 3 shared
responses in the \ac{cfs} to method section}



%
\textbf{A second} possible explanation, which is not mutually exclusive with the
previous explanation, could be that the audio-description paradigm does not
comprehensively sample the neural responses due to a sparser event structure.
%
...
to a visual paradigm, and thefore contributing to the lower estimation
performance.
%
...
%
These three participants with poor Cronbach's $\alpha$ also exhibited the lowest
correlation between the audio-description's predicted and empirical $Z$-maps.
%
...
%
When estimating the $Z$-maps of the visual paradigms based on data of the
...
identical to those evoked by the target paradigm, although it does require a
...
%
Untangling the individual contributions of the exclusively auditory stimulus,
the multi-paradigm \ac{cfs}, the transformation matrices of test subjects, and
the mean reliability of the audio-description's $Z$-maps on the estimation
performance presents a challenge.
%
However, considering that the within-paradigm prediction works "well" for most
subjects given a sufficient amount of data for alignment, the presence of
outliers does not significantly compromise the prediction performance.
%
Moreover, the observation that the average correlation between empirical and
predicted $Z$-maps surpasses Cronbach's $\alpha$ as more data from the
...
exclusively auditory experiment, such as a speech localizer, that yields more
reliable results in all subjects.
%
To test the hypothesis that the \ac{srm} filters out noise, one could calculate
more reliable second-level $Z$-maps from actual time-series and time series with
artificially added noise at different intensities.
%
...
%
If the \ac{srm} filters out noise, a prediction via an alignment based on a test
subject's time series with added noise should be more similar to the actual
$Z$-maps than the $Z$-maps calculated from noisy data.
...
activity in at least one subject during the visual localizer paradigm.
%
...
of the \ac{ppa} as defined by the visual localizer.
%
Consequently, the \ac{roi} introduced a bias against the reliability of the
$Z$-maps derived from the audio-description.
%
This is because the \ac{roi} includes voxels that do not demonstrate
significantly increased activity in at least one subject, but rather exhibit
random variation (or potentially significantly decreased activation, which would
not affect the reliability measure).


\subsection{Summary, shortcomings \& future studies}
%
In summary, our study revealed that using $\approx$\unit[15]{min} of the
audio-description for aligning a test subject to the \ac{cfs} resulted in
relatively low performance in cross- and within-paradigm prediction.
%
Nevertheless, we demonstrated the feasibility of However, the results suggested
that it is in principle feasibility of substituting a visual paradigm with an
auditory one which, however, limited by the amount of data necessary to be
collected.
%
An interesting avenue [?] for further investigation would be to estimate the
topography of a higher-auditory functional area using a narrative for functional
alignment and compare it with an estimation based on a movie.
%
...
visual localizer with higher fidelity than an estimation based on anatomical
alignment.
%
The prediction performance based on the movie significantly improves when
$\approx$\unit[30]{min} of the movie are used for alignment.
%
...
%
Overall, our results suggest that our functional alignment procedure is
approaching a performance limit, and longer scanning times may not yield
...
%
However, naturalistic stimuli offer a broader range of content compared to the
specific content used in the visual localizer paradigm.
%
While we focused on the \ac{ppa} as a higher-visual area in this study,
naturalistic stimuli have been successfully employed to investigate various
domains of brain function, including speech perception, music perception,
spatial cognition, perceived of experienced emotions, and theory of mind
...
experiments like the visual localizer paradigm used in our study.
%
...
brain processes that are not directly sampled during passive movie watching such
...
in-depth investigations into aspects that are beyond of scope in our study.
%
For example, we restricted our analysis to voxels in a \ac{roi} with an average
of $\approx$\unit[1600]{voxel} per subject.
%
The use of searchlight functional alignment \citep{zhang2016searchlight,
guntupalli2016model} could cover the entire cortex but would restrict the
functionally aligned voxels to those within the searchlight's sphere.
%
...
areas, or cases involving atypical topography resulting from cortical
reorganization due to brain injuries.

\todo[inline]{imo, connectivity functional alignment (again) fucks up that
argument}

\todo[inline]{check \citet{bazeille2021empirical}}
...
\todo[inline]{Vision should be located here; the general discussion is more
about pro (functional alignment, yeah!) \& contra of naturalistic stimuli}

\todo[inline]{repeat exact phrasings from intro (cf. localizer paradigms)}

\todo[inline]{Revise with (to be finalized) text about functional alignment in
the general discussion}

%
Our results suggest that $\approx$\unit[15]{min} functional scanning using an
engaging naturalistic stimulus could provide sufficient data for a functional
\textit{calibration scan}.
%
This calibration could be employed to align a new subject to a \ac{cfs} derived
from extensive scans of a reference group.
%
...
designed to reliably map brain processes related to low-level auditory or visual
perception, as well as higher cognition, such as theory of mind or
\citep{spunt2014validating} or semantic processes \citep{fedorenko2010new,
fernandez2001language}.
%
The diagnostic run would be a complex naturalistic stimulus that captures a wide
range of brain states, facilitating a valid alignment with the reference
\ac{cfs}.
%
...
%
Once a new subject is aligned with the \ac{cfs}, the functional data collected
from the reference group could be mapped through the \ac{cfs} into the new
subject's voxel space.
%
This would enables the estimation of regular patterns in the new subject when
obtaining additional functional scans is not feasible due to scanner
...
dissimilarity between a new subject's actual pattern (e.g., an empirical Z-map)
and the pattern estimated from a normative reference group.
%
...
\todo[inline]{This is just the "conclusion" generated by AI (and it's way too
long) after having seen the discussion}

\todo[inline]{I'll read it tomorrow and see if it makes sense to use it as a
template}

\todo[inline]{So, do not even bother to read it}

In conclusion, our study demonstrates the potential of using naturalistic
stimuli and functional alignment techniques to estimate brain activity patterns
in individuals based on a normative reference group.
%
We found that approximately 15 minutes of functional scanning with an engaging
naturalistic stimulus can provide sufficient data for a functional calibration
scan, allowing alignment to a common functional space (CFS) derived from a
reference group's scans.
%
...
Furthermore, our results highlight the importance of naturalistic stimuli in
eliciting responses across diverse domains of brain function.
%
...
investigation.
%
Future studies could consider alternative approaches, such as searchlight
functional alignment algorithms, to align voxels across larger distances,
particularly for functional areas with significant interindividual variability
or atypical topography resulting from brain injuries.
%
In summary, our study showcases the potential of functional alignment using
naturalistic stimuli and highlights the feasibility of estimating brain activity
patterns in individuals based on a normative reference group.  applications.

\paragraph{Just a shorter version}
...
reference group.
%
The aim is to provide an alternative approach to traditional controlled
paradigms for functional calibration and alignment.
%
...
%
This approach offers advantages such as higher engagement and better compliance
compared to controlled paradigms.
%
...
%
Naturalistic stimuli elicit responses across diverse brain functions, making
them valuable for investigating higher cognitive processes.
%
The study emphasizes the potential of functional alignment in other domains
beyond visual perception and suggests avenues for future research.
%
...
\subsection{"Problem": our matrices based on localizer generalize amazingly
well}
