...
probably do more of a denoising as opposed to modelling (reliable) outliers
incorrectly}
...
\todo[inline]{Somehow communicate general trends according to the
"three aspects under investigation" in the intro?}
...
\todo[inline]{discussion (somewhat) restructured according to "three aspects
under investigation"}
...
model/matrices; cross-paradigm prediction is more generalizability of
model/matrices}


\subsection{Short summary of aims \& hypotheses, method, results}

\todo[inline]{cf. summary at the beginning of the results section}

\todo[inline]{mention "within-paradigm" \& "cross-paradigm" prediction; cf.
intro's "Here, we..."}

%
For every fold of the cross-validation, we created a \ac{cfs} based on
time-series of three paradigms (i.e. a visual localizer, a movie, and the
movie's audio-description) that were concatenated before model fitting.
%
For every test subject, we use the test subject's response time
series of the visual localizer, movie, or the audio-description independently as
predictors to align the test subject to the respective \acp{tr} within the
\ac{cfs} and acquire the respective transformation matrices.
%
The $Z$-maps of every subject that were gained from previous analyses of the
three paradigms servered as critera that we estimated by projecting the
training subjects' respective $Z$-maps from their respective native voxel space
through the \ac{cfs} into a test subject's native voxel space.
%
Prediction based on anatomical alignment served as a benchmark.


\subsubsection{Aims / Hypotheses (a.k.a. three aspects under investigation)}

\todo[inline]{rephrase; cf. beginning of results where
it should be phrased better already; cf. "Here, we..." for within-paradigm and
cross-paradigm prediction}
%
...
\todo[inline]{short high-level summary here heavily depends on how the
discussion turns out to be}


\subsection{Validity \& generalizability of multi-stimulus model}

\todo[inline]{yeah, well, how to separate validity \& generalizability of
\ac{cfs} from validity \& generalizability of transformation matrices?}


\subsection{Validity \& generalizability of transformation matrices}


\subsubsection{Functional vs. anatomical alignment}

%
Movie (sooner or later) beats anatomical alignment when the visual paradigms
(i.e. localizer and movie PPA) are estimated.
%
...
to anatomical alignment.\todo{but why?}
...
%
It's pretty consistent across paradigms, too.



\subsubsection{Functional alignment across predictors \& criteria}

\todo[inline]{stick to present results; extend the idea to "lots of localizers"
below in "Future studies"}


The multi-paradigm model is kind of valid, I guess...


\paragraph{Validity \& generalizability of visual localizer}

\todo[inline]{why is prediction of localizer based on localizer runs
for alignment so "bad"?}

\todo[inline]{Is it our model or the transformation matrices?}

\todo[inline]{the localizer is "dedicated"; does not have a lot of \acp{tr} but
"a lot" and controlled events}
...
\paragraph{Templates from \citet{haxby2011common}}

\todo[inline]{In any case, our results are different from
\citep{haxby2011common}: \acf{cfs} \& transformation matrices based on visual
localizer that predict the localizer as good as / better than than \ac{cfs} \&
transformation matrices based on movie!}

\todo[inline]{if \citet{haxby2011common} that compare (within- and
across-paradigm) prediction based on naturalistic \ac{cfs} (more TRs) to
non-naturalistic \ac{cfs} (less TRs) is discussed (a lot), it should probably be
primed (in respect to within- and across-paradigm prediction) before "Here,
we..." in the introduction}

\todo[inline]{cf. long version at the very and of this chapter; here are the
...
\paragraph{Validity \& generalizability of movie (a.k.a. it's the best!)}

\todo[inline]{a.k.a. movie is best}

%
Results of \citet{haxby2011common} that suggest that \acp{cfs} derived from
traditional paradigms are of diminished generalizability to other paradigms,
presumably because traditional paradigms sample a sparser range of brain states
than naturalistic stimuli.
...
\paragraph{Validity \& generalizability of audio-description}

\todo[inline]{a.k.a. cross-modal!}

Kind of works, but you need a shit ton of data;
%
It gets better, the more data; in case of within-paradigm prediction it's better
than movie (and localizer, of course).




\subsection{Relationship between estimation performance \& quantity of data}

\todo[inline]{how to phrase? "asymptotic relationship", "reaching a ceiling"?}
...
\paragraph{Templates from other studies}

\todo[inline]{problem: \citet{feilong2022individualized} already did it (i.e.
assess data quantity); results suggest 30m are "good" in case of their
model; plus, they model individual component.}
...
\todo[inline]{imo, this is more a topic for the general discussion (pros \& cons
of naturalistic stimuli)}


\subsubsection{The Reliability of the audio-descriptions PPA}

\todo[inline]{following should also true (but less severe) in case of movie
PPA}

\todo[inline]{what's wrong?  the modeling in \citet{haeusler2022processing}? the
unbalanced number of events? Or does an auditory naturalistic stimulus give too
much room to participants to do not give a shit? a.k.a. freely listening is
bad?}

\todo[inline]{the "deviant" participants in the audio-description are
essentially the ones that have a poor Cronbach's: they are not "reliable
outliers" being different from the norm but noisy asses}

\todo[inline]{possible inference: SRM should probably do more of a denoising as
opposed to modeling (reliable) outliers incorrectly}
...
\paragraph{audio-description for (cross-paradigm) prediction}
...
\paragraph{Summary of: The Reliability of the audio-descriptions PPA}
...
\todo[inline]{This should not be relevant anymore; because the outliers in
prediction should be caused by shitty reliability not by the outliers being
reliably deviant from the norm}
...
\todo[inline]{don't write too much here; better discuss in general discussion
that naturalistic stimuli might be limited (e.g., might not sample executive
...
\paragraph{Obvious extension is a dataset with an additional localizer of
speech}
...
hyperalignment across different movie datasets; i.e no \ac{cfs} but one
transformation matrix for every pair of subjects (which is a shitty procedure to
scale, imo); they predict maps aimed to localize PPA, too}
