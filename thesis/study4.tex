...
\todo[inline]{similarly, in case the detailed procedure (s. below) gets shifted
from intro to method section, cross-validation is not mentioned in the intro;
having just done one last read: I think this should be done so I can speak of "a
test subject" and not just "the new subject"}

% focus: ppa
Here, we we focus on the \ac{ppa} \citep[e.g.,][for reviews]{epstein2014neural,
aminoff2013role}, and explore whether we can estimate the results of
$t$-contrasts that were created to localize the \ac{ppa} using functional data
of three different paradigms:
%
...
voxel in a subject's voxel space on each shared, and allows to functionally
align a subject's functional data to the \ac{cfs} by projecting responses within
...
a single paradigm, we calculate a \textit{multi-stimulus \ac{cfs}} based on
concatenated response time series from three paradigms (i.e., the visual
localizer, movie, and auditory narrative).
%
We then use a new subject's response time series to each of these paradigms
independently to align the new subject to the corresponding \acp{tr} within the
\ac{cfs} and acquire the subject-specific transformation matrix.


\paragraph{Three criteria}
%
In order to test the validity and generalizability of our multi-stimulus
\ac{cfs} and the transformation matrix of a new subject, we estimate the
$t$-contrast's results \citep[i.e. from the visual localizer, the movie, and the
auditory narrative; cf.][]{sengupta2016extension, haeusler2022processing} of
each of the three paradigms (i.e. within-experiment and cross-experiment
prediction).


\paragraph{Partial alignment}
%
Further, since a complete naturalistic stimulus to align a new subject is
impractical in a clinical setting, we explore the relationship between the
estimation performance of the $t$-contrast's results and the quantity of data
from each of the three paradigms used to functionally align the subject to the
...
procedure procedure to the performance of a volume-based, anatomical alignment
procedure that serves as a benchmark.


\subsubsection{Our functional alignment procedure}

\todo[inline]{In case the part above is sufficiently detailed for the
introduction (mention mask and cross-validation?), the following part might me
shifted to method section as an overview (adjusted and shortened before
"Modeling of the common functional space (CFS)")}

\todo[inline]{if this part gets shifted; intro is not mentioning
cross-validation}

%
Following an exhaustive leave-one-out cross-validation, we estimate results of
three criteria  using a volume-based, functional alignment procedure, and
...
%
Our approach to estimate a test subject's localizer result (i.e. a test
subject's \textit{empirical $Z$-map}) via functional alignment is conducted in
four steps.
...
data (of the same!) paradigm based on simplified stimuli.esults show...
