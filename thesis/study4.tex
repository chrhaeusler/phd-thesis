...
\todo[inline]{Summarize / merge the templates}
...
\todo[inline]{A couple of sentences should be enough; state again the
...
\subsection{Overview of performed comparisons}


\todo[inline]{Here or per estimated criterion (s. below), give a short overview
of the performed and reported tests (otherwise it will probably be confusing to
read)}

\todo[inline]{Explain here or somewhere else why which tests have (not) been
chosen, while some findings, i.e. the "general trend", will be discussed based
on eye-balling?}

\todo[inline]{So far, I did not test difference(s) to Cronbach's}


\subsection{General stuff concerning all the following tests}

\todo[inline]{Correlations were Fisher's z transformed, tested for Normality via
Shapiro-Wilk (imo, the most appropriate test here)}

\todo[inline]{in case Shapiro-Wilk was not significant (failed to reject H0 of
normality), an dependent [?!] t-Test was performed, otherwise (non-parametric)
Wilcoxon signed-rank test (for dependent samples) should be computed?}

\todo[inline]{Bonferroni correction: set alpha level according to the (final?)
number test}

\todo[inline]{Report descriptive statistics of the correlations? imo, that's not
necessary}

Descriptive statistics are reported for the "raw" correlation values; before
performing statistical tests of differences, values Fisher z-transformed, and
...
\subsection{Estimation of the visual localizer's empirical $Z$-maps}

\todo[inline]{Report mean correlations and standard deviations?; imo, it gets
pretty crowded, can inferred from the stripplot, and they get Fisher transformed
anyway before testing}
...
Shapiro-Wilk test -> non parametric test (Wilcoxon signed-rank should be the
correct one)?}

\todo[inline]{Round the values of just write $p$<.0001}
...
\todo[inline]{Text is simply copied from above but adjusted according to the
movie as criterion}
...
\todo[inline]{Which tests to perform? Stripplot is a mess; merely eyeballing
feels unsatisfactory; essentially all samples will fail the test for normality
}
