...
%
We found that using a partial alignment approach with a \ac{cfs}
...
align a test subject with the \ac{cfs}.
%
However, $\approx$\unit[15]{min} of movie data outperformed an estimation using
...
\acp{cfs}, one \ac{cfs} based on data of a controlled paradigm that employed
stimulus categories presented in still images and another \ac{cfs} based on
movie data.
%
Results revealed a higher cross-subject classification of the traditional
paradigm's stimulus categories via the \ac{cfs} and transformations calculated
based on data of the same paradigm compared to a cross-subject classification
via the \ac{cfs} and transformations calculated based on the movie data.
%
...
sampling the classified brain states more extensively than the movie.
%
...
are used, while also closing the gap with the prediction based on the same
amount of movie data.
%
Comparing the prediction based on $\approx$\unit[120]{min} of the
...
paradigm's results.
%
However, it would require a long functional scanning session to achieve a
...
%
The within-paradigm prediction based on $\approx$\unit[15]{min} of the
...
%
Given the exploratory nature of this study, pinpointing the exact reasons for
...
%
\textbf{A first} explanation might be inferred from the fact that the amplitudes
of the shared responses during the \acp{tr} of the audio-description are lower
than during the \acp{tr} of the movie and visual localizer.

\todo[inline]{cf. plot of TRs of top 3 shared responses in the \ac{cfs}; add it
to method section?}

\todo[inline]{which leads to what inference exactly?}

%
\textbf{A second}, not mutually exclusive, explanation might be that the
audio-description does not sample the responses as comprehensively due to a
sparser event structure, which becomes particularly pronounced when estimating
the results of visual paradigms.
%
\textbf{A third}, again not mutually exclusive, explanation might be that the
responses in the \ac{ppa} to an auditory paradigm differ from responses to a
visual paradigm, which could contribute to the lower estimation performance.
%
...
difficult to experimentally or statistically control during naturalistic
stimulation.


%
In the present study, calculating Cronbach's $\alpha$ of the audio-description's
$Z$-maps revealed mediocre to poor reliability in three subjects.
%
When estimating these empirical $Z$-maps, the three participants with poor
Cronbach's $\alpha$ also showed the lowest correlation between predicted
and empirical $Z$-maps.
%
Results suggest that these participants are not outliers exhibiting a response
pattern that is reliably deviant from the norm but are outliers exhibiting more
variable responses to auditory spatial information over the course of a two-hour
long stimulation.
%
In the remaining number of subjects, the prediction performance improves, both
in within-paradigm and cross-paradigm predictions.
%
This suggests that an auditory narrative sufficiently samples responses vectors
that can be used to align a test subject to a \ac{cfs} and acquire
transformations that enable the estimation of a visual paradigm's results.
%
These findings are line with \citet{haxby2011common} who found that the
general validity their \ac{cfs} ``based on the responses to the movie is not
dependent on responses to stimuli that are in both the movie and the category
perception experiments but, rather, appears to rest on stimulus properties that
are more abstract and of more general utility'' \citep{haxby2011common}.
%
In light of our current findings, we assume that a stimulus used for functional
alignment does not necessarily sample hemodynamic responses that match responses
evoked by the to be predicted paradigm, however with the drawback of a long
scanning time.
%
Therefore, it would be interesting to explore if functional alignment can in
principle be employed to estimate $Z$-maps from paradigms that were designed to
elicit brain processes that are "probably" not sampled during passive movie
watching such as planning or decision-making.

%
Disentangling the degree to which the exclusively auditory stimulus, the
multi-paradigm \ac{cfs}, the transformation matrices of a test subject, and the
mean reliability of the $Z$-maps influence the estimation performance is
challenging.
%
However, given that the within-paradigm prediction works "well" for most
subjects given sufficient amount of data to align the subject, the time series
of the outliers do not significantly diminish the validity of the model and
suggest that the \ac{srm} is robust against outliers.
%
...
the audio-description are used, could also be interpreted as the \ac{srm}
denoising a test subject's data \citep[cf.][]{chen2015reduced}.
%
...
%
In order to test the hypothesis that the \ac{srm} filters out noise, a more
reliable second-level $Z$-maps could be calculated from actual time-series and
time series with added artificial noise at different levels of intensity.
%
...
\todo[inline]{does that makes sense to you??? take the noisy time series for
alignment of test subject; does procedure better predict the $Z$-maps calculated
from actual time-series or $Z$-maps calculated from noisy time-series?}

%
Lastly, it is important to acknowledge that we restricted the analysis to voxels
located in the union of voxels that showed a significantly increased activity in
at least one subject during the visual localizer paradigm.
%
...
%
Therefore, the \ac{roi} created a bias against the auditory paradigm when
calculating the reliability of the audio-description's $Z$-maps because the
...
%
Nevertheless, results suggest that it is in principle possible to use an
auditory paradigm to predict results of a visual paradigm.
%
...
alignment and compare this estimation to the estimation based on movie data.
%
...
%
Overall, our results indicate that our functional alignment procedure approaches
a performance limit, and longer scanning times do not necessarily provide a
substantial benefit.
...
%
Future studies should investigate the performance of functional alignment in
other domains than higher-visual perception.
%
...
%
Lastly, it needs to be stressed that our study is exploratory, based on a sample
of 14 subjects, and therefore provides preliminary results that invite further
studies that can dig fucking deeper into different aspects that are fucking out
of scope in our study.
%
For example, we restricted our analysis to voxels in a \ac{roi} with an average
count of $\approx$\unit[1600]{voxel} per subject.
%
Employing a searchlight functional alignment \citep{zhang2016searchlight,
guntupalli2016model} could cover the whole cortex but would restrict the
functionally aligned voxels to those within the sphere of the searchlight.
%
Future studies are necessary that develop functional alignment algorithms that
allow to align voxels across larger distances.
%
Such algorithms would be especially beneficial for functional areas whose
anatomical location is known to vary greatly across subjects such a language
areas or for cases of atypical topography, for example, after cortical
reorganization resulting from brain injuries.
...
\subsubsection{Some templates for "whole-brain vs. ROI vs. searchlight"}

\footnotesize

\todo[inline]{sphere size / distance in \citet{guntupalli2016model} and \citet{haxby2011common}?}
...
%
A calibration could be used to align a new subject to a \ac{cfs} that was
...
%
The diagnostic run would be based on a complex naturalistic stimulus that
...
difference of a new subject's actual pattern (e.g., an empirical $Z$-map) to a
pattern estimated from a normative reference group.
%
For instance, a recent study by \citet{yates2021emergence} calculated a \ac{srm}
to investigate the presence and localization of adult brain functions in
children.
%
...
movie.
%
They found reliable correlations between the children's predicted and actual
\ac{fmri} activity, and the strength of the correlations in the precuneus,
