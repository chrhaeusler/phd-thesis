...
\todo[inline]{focus on localizer PPA (also in length of the text)}


\subsection{Introductory paragraph}

\todo[inline]{(implicitly) define "predictors", "criteria", "multi-paradigm
\ac{cfs}" again}

\todo[inline]{shift results / conclusions here to actual conclusions?}
...
\subsubsection{Longer version of "Approach"}
%
...
First, we estimated the results of the visual localizer which is the established
method to identify the \ac{ppa}.
%
Our results show that $\approx$\unit[15]{min} of localizer data or
$\approx$\unit[15]{min} movie data used to functionally align a test subject to
a \ac{cfs} are sufficient to estimate the results of the visual localizer with
higher fidelity than an estimation procedure based on an anatomical alignment.
%
Comparing the within-paradigm prediction based on $\approx$\unit[15]{min} of
localizer data to the cross-paradigm-prediction based on $\approx$\unit[15]{min}
of movie data revealed a higher estimation performance of the within-paradigm
prediction.
%
...
$\approx$\unit[30]{min} of the movie are used to align a test subject with the
\ac{cfs}.
%
However, comparing $\approx$\unit[30]{min} to $\approx$\unit[45]{min} to align a
test subject with the \ac{cfs} yield[ed] no difference.
%
Overall, results suggest  that the estimation based on functional alignment
...
based on an auditory narrative lacking a visual stimulation.
%
The prediction based on data of the audio-description revealed the lowest of all
alignment procedures, including anatomical alignment.
%
The prediction performance based on the audio-description increases the more
data are used, and closes the gap to the prediction based on the same amount of
movie data.
%
Comparing the prediction based on $\approx$\unit[120]{min} of the
audio-description to the prediction based on an anatomical alignment revealed no
statistical difference.
%
Results suggest that an auditory paradigm could, in principle, substitute a
visual paradigm for functional alignment and a subsequent estimation of a visual
paradigm's results, however at the cost of a long functional scanning session
needed to acquire a comparable estimation performance.
...
\todo[inline]{cf. \citet{haeusler2022processing}: AO response (too) different
from AV / VIS?}

\todo[inline]{cf. plot with TRs of shared responses: lower amplitude}

%
Second, we estimated the results of \citet{haeusler2022processing}, where we
built $t$-contrast aimed at localizing the \ac{ppa} based on modeled hemodynamic
activity based during two naturalistic stimuli.
%
When estimating the results of the naturalistic paradigms that served as an
alternative way to localize the \ac{ppa}, the within-paradigm predictions based
on functional alignment lead to the highest correlations between empirical and
predicted $Z$-maps.

%
Similar to the estimation of the visual localizer's results, estimating the
results of the movie via the audio-description revealed the lowest estimation
performance of all alignment procedures, including anatomical alignment.
%
The prediction performance improves the more data of the audio-description are
used to align the test subjects and the gap to the estimation performance to the
alignment using the same amount of movie data narrows.
%
These results suggest the in principle usability of an exclusively auditory
paradigm to estimate the results of a visual paradigm, however at the expanse of
an extensive data collection.


%
Moreover, the within-paradigm prediction based on $\approx$\unit[15]{min}
audio-description to predict the $Z$-maps from the audio-description did not
yield a better performance than a prediction via anatomical alignment.
%
In case of the within-paradigm prediction it should be noted that the $Z$-maps
of from the audio-description have the lowest reliability.

%
Given the exploratory nature of the present study, tracking down possible
reasons are difficult.
%
However, three interpretations seems feasible.
%
%
A feasible reasons might be that the audio-description is ``simply not as rich''
as the movie and does not sample the response vector space as much as the movie,
and issues that is aggravate when estimating the results of the visual paradigms
because the auditory fucking lacks a visual simulation.
%
Another, again not mutually exclusive explanation, might be that the response in
the \ac{ppa} to an auditory paradigm is (too) different from a response in the
\ac{ppa} to a visual paradigm.
%
A third, again not mutually exclusive, reason might be that the amplitudes of
the shared responses during the \acp{tr} of the audio-description are lower than
during the \acp{tr} of the movie and visual localizer.
%
It is hard to judge disentangle the influence of the model, transformation
matrices of the test subject and the estimated criterion.
%
The performance gets better the more data are used which is the case for the
within-paradigm (gets higher than Cronbach's) and cross-paradigm prediction.
%
So, the model seems to be valid and use of matrices and model generalizes to the
visual paradigms.
%
It would be interesting to estimate the results from another controlled
same-modality criterion (e.g. speech localizer).


\paragraph{Support for \citet{haeusler2022processing}}
%
Present results in be interpreted as support of \citet{haeusler2022processing}
where we found hemodynamic activity during auditory stimulation that is
restricted to the anterior part of the localizer PPA are not based on the
methodological decisions, and that the responses to auditory spatial information
lead to different activity patterns than visual stimulation.
%
...
\paragraph{ROI favors visual localizer}
%
The fact that the correlation mean correlation between empirial and predicted
$Z$-maps based on $\approx$\unit[15]{min} exceeds the Cronbach's could be
interpreted as denoising a test subject's data.\todo{bold, not statistically
tested anyway}
%
The results are an incentive for future studies to develop a procedure that more
reliably maps a functional area based on annotations of a naturalistic,
preferably exclusively auditory, stimulus.
%
A more reliable criterion could than be estimated based on actual data used for
alignment and compared to data with added artifical noise to investigate if the
\ac{srm} filters out the noise.


\todo[inline]{cf. \citet{cohen2017computational, chen2015reduced}}


\paragraph{Final statement before summary}
%
Lastly, it should be noted that we restricted the voxels to voxels located in
the union of voxels that showed an significantly increased activity in an least
one subject during the visual localizer paradigm leading to a tacit assumption
that the localizer, movie and audio-description lead to increased activity in
the same set of voxels.
%
Given that results of \citet{haeusler2022processing} suggest that hemodynamic
activity during auditory semantic stimulation is more restricted to the anterior
part of the \ac{ppa} defined by the visual localizer, the reliability measure
disfavors the auditory paradigm because the \ac{roi} comprises more voxels
compared to the localizer that do not exhibit significantly increased activity
in at least one subject but random variation (or potentially decreased
activation, but then it's less of a reliability measure).


\subsection{Interim summary}

\todo[inline]{visual localizer generalizes unexpectedly well}

\todo[inline]{general opinion of naturalistic stimulus is hyped?}

\todo[inline]{we need more studies that do other stuff than visual-areas}


%
In summary, audio-description has comparatively low cross- and within-paradigm
prediction when merely $\approx$\unit[15]{min} minutes are used to align a test
subject.
%
However, results sggest that it is in principle possible to use an auditory
paradigm to predict a visual paradigm.
%
Nevertheless, the sutability to sibstitute a visual paradigm is limited by the
amount of data necessary to be collected during listening to an
audio-description to potentially estimate a visual category-selective area.
%
An obvious point for further investigatiom that estimates estimates the
topography of an higher auditory functional area via an narrative and compare
this estimation to an estimation done via an audio-visual movie used for
functional alignment.

%
Our results provide evidence that $\approx$\unit[15]{min} of movie
data used to functionally align a test subject to a \ac{cfs} are sufficient to
estimate the results of the visual localizer with higher fidelity than an
estimation procedure based on an anatomical alignment.
%
The prediction performance based on movie data increases significantly when
$\approx$\unit[30]{min} of the movie are used to align a test subject with the
\ac{cfs}.
...
test subject with the \ac{cfs} yield[ed] no difference.
%
Overall, results  that the estimation based on functional alignment approaches a
performance limit and a decreasing benefit of longer scanning time (cf.
Fig.~\ref{fig:stripplot}).
%
The movie has lower prediction performance than the within-prediction but
potentially generalizes better / to more paradigms than the visual localizer.

\todo[inline]{add here why movies are so versatile}



\subsubsection{Future studies: other ROIs than category-selective areas}
...
\paragraph{Intro:}
%
Admittedly, we focused on the localizer \ac{ppa} as it is an established
methods to reliably localize a higher-visual area in the majority of subjects.
%
In the present study, we focused on the \ac{ppa} as a classic
example of a higher-visual area.

\paragraph{Previous studies}

Low between-subject variability as found in retinotopically defined regions
early visual cortex \citep{rosenke2021probabilistic} compared to high
between-subject variability as found in language areas in compared to language
areas \citep{frost2012measuring}.
...
when estimating other functional areas with different levels of improvement
compared to an anatomical alignment.


\paragraph{Movies are (said to be) super fucking awesome}
%
...
\todo[inline]{cite review summarizing range of brain functions studied during
naturalistic stimulation}

Hence, further studies should investigate other functional areas, esp. other
domains (a.k.a. no higher-visual areas)

Interesting would be to test how far you can take the movie to generalize to
other non-visual localizer;
%
...
%
Compare performance of narrative vs. movie when estimating speech localizer.
...
\subsubsection{"Naturalistic" localizer = difficult; thefore estimate
established one}
