...
\todo[inline]{explain steps; cf. introduction}
\todo[inline]{input data: 'ao \& av \& vis'; but also tested 'ao \& av'}
...
%
\todo[inline]{s. script 'get\_wmatrix\_for\_left-out.py'}

\todo[inline]{check code of the object's method in BrainIAK docs whether it uses
Procrustes transformation; iirc, it's the case}
...
\todo[inline]{i.e.: in both cases the mean was computed in subject-specific
anatomical space; this is more 'similar' than computing the mean in anatomy for
prediction from anatomy, and computing the mean in CMS for prediction from CMS;
but: creating the template in CMS does not lead to different results anyway}
...
\todo[inline]{imo, plots generated by that script are not very informative}
%
...
\todo[inline]{current version of script correlates AO regressors with shared
responses during the AO TRs (AND wrongly uses 0:3599 TRs)}

\todo[inline]{further, change from model using 'ao-av' to 'ao-av-vis'}
