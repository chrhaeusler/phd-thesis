...
To address the challenge of functional-anatomical variability across persons,
    ...
    derive[d] from the concatenated responses to the localizer, the movie and
...
% results 1
    Our results reveal that data from the audio-description allow[ed] to
    estimate the results of the visual paradigms, however at the expense of a
...
% discussion
This opens up the possibility of estimating results from many localizers using a
    a movie of similar duration to that of one traditional localizer.
...
In the domain of high-visual perception, functionally defined
...
of each predictor used to obtain a test subject's transformation and
...
%
In our study, we focused on \ac{ppa} as an example of a high-visual area.
%
...
the training subjects' $Z$-maps were projected via a non-linear, volume-based
...
naturalistic stimulus of similar duration to one traditional localizer paradigm.
...
\acp{cfs}:
%
one \ac{cfs} based on a controlled paradigm that employed images of
