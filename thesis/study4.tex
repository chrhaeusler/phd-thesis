...
of \unit[100]{s}), and every segment of the movie and audio-description (cutoff
...
2.5]{mm}), (c) the average number of voxels per \acp{roi}, (c) and findings from
...
\todo[inline]{subjects are aligned via SVD solving the Procrustes problem;
rephrase the paragraph (starting at "we let the algorithm learn an orthonormal
mapping...") }
...
%
For the time series of each paradigm separately, we let the algorithm learn an
orthonormal\todo{check that} transformation matrix $W_{n}$ that performs a
mapping of the responses in the test subject's voxel space during the respective
paradigm into the \ac{cfs}.
%
...
\todo[inline]{Provide (some of the) descriptive statistics in the text? at the
moment, there are just some numbers without real text because I think that plot
and table are more than sufficient}
...
\todo[inline]{audio-description's outlier: sub-05 (\textbf{-0.5!!}), sub-02 (0.0), sub-20
