...
\todo[inline]{description of \citet{haxby2011common}'s might be too long}
...
\todo[inline]{I have a plot of (just the) shared features but across all TRs
within the \ac{cfs}; "problem" in the plots shown below: the correlations
...
\todo[inline]{What is the point to make with the plots as support?}
...
%
Correlation of shared features (occurring during TRs of the visual localizer)
with modeled responses (regressors) used in \citet{sengupta2016extension}: see
Fig.~\ref{fig:corr-vis-reg-srm}.
%
Correlation of shared features (occurring during TRs of the movie) with modeled
responses (regressors) to visual stimulus features as used in
\citet{haeusler2022processing}: Fig.~\ref{fig:corr-av-reg-srm}.
%
Correlation of shared features (occurring during TRs of the audio-description)
...
\includegraphics[width=\linewidth]{figures/corr_vis-regressors-vs-cfs_sub-01_srm-ao-av-vis_feat10-iter30_7123-7747.pdf}
    ...
    and the corresponding \acp{tr} shared features within the \ac{cfs}
...
\begin{figure*}[tbp] \centering
    ...
    Grey dots:
    %
    correlations between empirical $Z$-maps and an estimation using
    ...
    Green dots:
    %
    correlations between empirical $Z$-map and an estimation using
    functional alignment based an transformation matrices
    calculated from one up to four runs
    (each lasting \unit[5.2]{m})  of the visual localizer.
    %
    Red dots:
    %
    correlations between empirical $Z$-map and an estimation using
    functional alignment based an transformation matrices
    calculated from one up to eight segments
    (each lasting $\sim$\unit[15]{m}) of the movie.
    %
    Blue dots:
    %
    correlations between empirical $Z$-map and an estimation using
    functional alignment based an transformation matrices
    calculated from one up to eight segments
    (each lasting $\sim$\unit[15]{m}) of the audio-description.
    }
    \textbf{
        ToDo: Legend for mean/median Cronbach's of empirical Z-maps;
        keep boxplots (and median \& mean)?
        }
