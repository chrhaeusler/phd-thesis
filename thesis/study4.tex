...
When estimating the visual localizer, using $\approx$\unit[15]{min} of localizer
data (within-paradigm prediction) or $\approx$\unit[15]{min} (\ac{tr}=2s) of
movie data (cross-paradigm prediction) to align a test subject to a \ac{cfs} and
estimate $Z$-maps of the localizer outperform a procedure based on anatomical
alignment.
%
However, the $\approx$\unit[15]{min} of localizer data are better than
$\approx$\unit[15]{min} (\ac{tr}=2s) of movie watching.
%
\textbf{Inference:} in line with results from \citep{haxby2011common}.

%
Prediction performance via movie data further when
...
stimulus.
%
\textbf{Inference:} efficiency and stuff (cf. introduction); movie is awesome:
immersive and no task; worse than within-prediction but acceptable loss of
prediction performance; possibly applicable to lots of other (visual?)
paradigms.

%
We also tested the prediction of the visual localizer via a non-visual
...
closing the gap to the prediction based on movie data of the same amount of
data.
%
...
%
\textbf{Inference:} In general it works, but results suggest a lot of data are
necessary to acquire a comparable estimation performance that could be obtained
by mere anatomical scans.
...
%
Similar pattern for movie and audio PPA: within-paradigm prediction performs
best;
%
movie generalizes better than visual localizer to the audiory paradigm.
%
auditory paradigm generalizes moderately to the movie:
%
as in the visual localizer paradigm, estimating the results of the movie via the
audio-description revealed the lowest estimation performance of all alignment
approaches.
...
%
Another explanation is that the response in the PPA to an auditory paradigm is
(too) different from a response in the PPA to a visual paradigm.

%
The more data are used the more the gap closes, suggesting the in principle
usability of an exclusively auditory paradigm to estimate the results of a
visual paradigm, however at the expanse of an extensive data collection.
...
%
This is different from the other two within-pardigm prediction via the visual
paradigms.
%
It should be noted, however, that the audio PPA has the most outliers and maybe
the shared responses are not that representative / clear and the functional
alignment has way more degrees of freedom than the anatomical alignment.
%
In any case, the prediction performance if audio-description and movie get
better the more data are used.
%
It is hard to judge disentangle the influence of the criteria, the validity of
the model and the validity of the transformation matrices.
%
The model seems valid because the performance exceeds Cronbach's alpha of the
criterum.
%
...
persons is particularly high, as in atypical organization in e.g. language areas
or brain disease in general.
...
\subsection{Problems with naturalistic stimuli}
