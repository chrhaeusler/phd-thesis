...
\todo[inline]{mih: many points of the discussion should be primed in the intro
to better frame the problem space, challenges \& opportunities}
...
\todo[inline]{Try to merge \citet{weiner2018defining} into text above; i.e. give
one general example (that includes PPA) and one that just focusses on PPA}

\todo[inline]{What did \citep{frost2012measuring} specifically say about PPA?}
...
\todo[inline]{check \citet{jiahui2020predicting}'s video localizer, studies that
the localizer cites and new studies that that the localizer}
...
\todo[inline]{add here that alignment uses whole stimuli and that this is
inefficient}
...
\begin{itemize}
    \item in general: one "expensive" model, one "affordable" calibration scan
    \item prediction of localizer from reference (group)
...
\todo[inline]{if messages are more clear, some captions in discussion are
redundant}
...
shift to discussion: naturalistic is awesome...

\begin{itemize}
    \item for fitting them model (cf. literature)
    \item also for the alignment!
    \item and therefore for the prediction
\end{itemize}
...
\todo[inline]{Probably drop that point}
...
\subsection{We created a common functional space from...}
...
\subsection{Tacit assumption that localizer = ground truth}
...
\subsection{Why partial alignment is awesome}
