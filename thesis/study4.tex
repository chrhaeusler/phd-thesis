...
%
The topographies (i.e. the location, size and shape) of category-selective areas
are similarly distributed across individuals but the exact location, size and
shape varies [considerably] \citep{rosenke2021probabilistic,
...
contrast regressors representing modeled hemodynamic responses to presented
...
\subsection{Recapitulation of study 2}
...
\subsubsection{Estimation from reference group}
...
A traditional procedure \citep[e.g.,][]{frost2012measuring, weiner2018defining,
...
area in a person's anatomy from a reference group performs a volume-based or
surface based alignment \textit{anatomical alignment}.
%
In order to resolve [the issue of] anatomical variability across persons,
...
\paragraph{Volume-based and surface-based alignment}
% volume-based
...
Surface-based anatomical alignment \citep{fischl1999cortical,
fischl2012freesurfer, yeo2009spherical} aligns vertex-wise data of individuals
to a two-dimensional template \citep[e.g., FreeSurfer fsaverage
template;][]{fischl1999high}.
%
Whereas volume-based alignment does not account for interindividual variability
in sulcal and gyral folding patterns, surface-based alignment respects folding
pattern of the cortical surface [Fischl et al., 2008; Hill et al., 2010; Van
Essen and Dierker, 2007][references taken from \citet{zhen2017quantifying}].


\paragraph{Surface-based works better}

\todo[inline]{all three studies \citep{frost2012measuring,
rosenke2021probabilistic, weiner2018defining} compare surface-based to linear /
affine (!) volume-based alignment}

\todo[inline]{Merge all studies into one point}

% Frost: regions
\citet{frost2012measuring} localized 13 functional areas of the high-level
visual cortex and ``found a large variability in the degree to which functional
areas respect macro-anatomical boundaries across the cortex''
\citep{frost2012measuring}.
% Frost: results
\citet{frost2012measuring} compared a piece-wise linear volume-based alignment
to a surface-based alignment, and found that surface-based alignment reduces
variability of functional areas across study participants.

% Rosenke
Similarly, \citet{rosenke2021probabilistic} have shown cortex-based alignment
reduced between-subject variability ``in the majority of functionally defined
ROIs [which ones] compared to nonlinear volumetric alignment [NVA]''
\citep{rosenke2021probabilistic}.

% Weiner
\citet{weiner2018defining} showed that ``the implementation of CBA significantly
improved the alignment and predictability of place selectivity within medial VTC
across participants compared to Talairach alignment''
\citep{weiner2018defining}.


\paragraph{Remaining variability: functional-anatomical correspondence}
...
\textit{functional--anatomical correspondence} of a brain function's location in
respect to the underlying anatomy.
...
%
Since anatomical alignment resolves anatomical variability but does not address
the issue of functional-anatomical variability across subjects, algorithms ---
like \textit{hyperalignment} \citep{haxby2011common, guntupalli2016model} or the
...
%
Whereas anatomical alignment aligns voxels (or surface vertices) that share the
same anatomical location to a \textit{common anatomical space}, functional
alignment aligns voxels (or surface vertices) that share similar functional
properties to a \textit{common functional space} (CFS).
%
Functional alignment algorithms are usually used to construct both the \ac{cfs}
(serving as ``a high-dimensional, functional brain template'') as well as
subject-specific transformation matrices.
%
A subject's transformation matrix can be used to perform a linear mapping of
functional data from a subject's anatomical space into the \ac{cfs}, or to
project data from the \ac{cfs} into a subject's anatomical space
...
In order to resolve [the issue of] functional-anatomical variability across
...
based on the \ac{srm} algorithm \citep{chen2015reduced, richard2019fast}.
...
\todo[inline]{"mathy" stuff is in method section}
...
\todo[inline]{very shortly state how anatomical alignment is performed; cf.
general introduction \& introduction above}
...
\todo[inline]{results: SRM fails in case a subject has atypical results in study
2 (i.e. subjects has no auditory PPA) independent of which naturalistic stimulus
...
\todo[inline]{no hypothesis regarding alignment via localizer runs to
predict localizer}

\todo[inline]{no hypothesis regarding prediction of "auditory PPA"}
...
\todo[inline]{Plots of correlation matrices should be presented here, not in
results (as in the PPA paper)}
...
shared responses change depending the 13 subjects used for each test subject to
calculate the model}
...
%
However, ``results showed that the sampling of response vectors from these
...
\todo[inline]{I am here}

%
Previous studies have shown that transformations for functional alignment have
greatly diminished general validity \citep{haxby2011common}, presumably because
such experiments sample a sparser range of brain states
\citep{guntupalli2016model}'' [project proposal].
%
Transformations for functional alignment that were derived from data of
naturalistic stimuli have been shown to be of increased validity, presumably
because naturalistic stimuli sample a more diverse set of mental states (e.g.
visual or auditory perception, spatial cognition; emotion; music, speech or
social perception).

%
Naturalistic stimuli sample a broader range of brain states than paradigms
...
%
``rich variety of stimuli or conditions are necessary to  .rich set of data that
samples a wide variety of cortical patterns (in the response vector space) in
order to generalize to novel stimuli or tasks'' \citep{haxby2020hyperalignment}.


\paragraph{Conclusion}

%
Hence, we also let the algorithm learn the orthogonal mapping to the \ac{cfs}
based on a test subject's functional data of the localizer experiment in order
to compare the estimation performance based on matrices derived from
naturalistic stimuli and a paradigm using simplified stimuli.
%
We used one up to four runs (each lasting $\sim$\unit[312]{s}) of the localizer
acquire four transformation matrices based on an increasing quantity of data
...
\todo[inline]{I also tested averaging data in \ac{cfs}: similar results; in case
of anatomical alignment, I did not test averaging data in MNI152 space}
...
\todo[inline]{probabilistic maps have problems with overlapping areas; strict
binary ROIs / parcellation does not make sense, imo, anyway; they do not capture
the idiosyncrasies of the maps within a ROI in an individual subject}
...
% suface-based alignment works better with retinotopic areas than
...
``it is unclear how our method, developed using data from a small subset of the
