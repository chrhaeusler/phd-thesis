...
\subsection{Functional vs. anatomical alignment}
...
to anatomical alignment; which is also the best estimated paradigm of the visual
localizer. \todo{but why???}
...
predictor; i.e. per predictor, discuss each criterion and the quantity of data}


The multi-paradigm model is kind of valid, I guess...


\subsubsection{Validity \& generalizability of movie (a.k.a. it's the best!)}

\todo[inline]{a.k.a. movie is best}

\todo[inline]{order by criteria: localizer, movie PPA, audio PPA}
%
...
\paragraph{Templates for "data quantity" from \citet{feilong2022individualized}}
...
\todo[inline]{cf. template at the very end of this chapter}
...
\todo[inline]{a.k.a. it's different previous studies!}

\todo[inline]{order by criteria: localizer, movie PPA, audio PPA}


\paragraph{audio-description to estimate the visual paradigms}
%
...
data are available
%
It's cross-modal in case visual paradigms are supposed to be estimated.

%

%
Audio-description is lacking visual stimulation and is obviously "not-as-rich"
as the movie;
%
Additionally, maybe, the auditory response is (too) different from visual response?

%
Audio-description gave more room for individual variation of responses (cf.
reliability of the audio-PPA-contrast in some subjects)?

\paragraph{audio-description to estimate the auditory PPA}
...
Obviously, interesting would be estimation of results from a controlled speech
paradigm (=different same-modality criterion).
...
\subsubsection{audio PPA as criterion = issue of reliability}
...
Issues of methodological decisions:
%
We might have modeled the time course of "spatial responses" "insufficiently" in
...
%
Generally, we should have modeled the construct "correctly" for >50\% of
subjects in \citet{haeusler2022processing}.

Issues of the paradigm:
%
Results of \citet{haeusler2022processing} also suggest that the
audio-description generally samples the response vector(s) that carry the
spatial response information.
%

The plots of shared responses \& AO regressors (kind of) suggest that the shared
responses during \acp{tr} of the audio-description are not total garbage.
%
Also, within-paradigm prediction kind of works for most subjects
%
Which also suggest, that the Cronbach's-a-outliers do not degrade the model too
much.

%
Did the audio-description give too much room for individual variation of
responses?
%
Issue of auditory domain in general?
%
Or an issue of a 2h-long auditory-stimulation that is not as immersive /
engaging as flashy movie frames?
%
Is the number of events across segments too unbalanced?


Does an auditory naturalistic stimulus give too much room to participants to do
not give a shit?
%
Is freely listening "bad"?
%
In case of audio-description attentional focus is hard to judge / control (cf.
eye-tracking during movie); alertness (via EEG) should be possible though.
...
\todo[inline]{just some random ideas}
