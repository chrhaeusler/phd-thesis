...
\paragraph{Intro}
%
A traditional procedure \citep{rosenke2021probabilistic, weiner2018defining,
zhen2017quantifying, wang2015probabilistic, frost2012measuring} [preliminary
citation; just take the best matching] to estimate a person's functional
topography from a reference group starts by anatomically aligning a person's
structural and functional data to a \textit{common anatomical space} (i.e. a
template) in order to resolve anatomical variability across persons.
% volume-based
...
%
Previous studies \citep{rosenke2021probabilistic, weiner2018defining,
zhen2017quantifying, wang2015probabilistic, frost2012measuring} [same refs as
above] have shown that ....
...
%
The ``Variability in functional-anatomical correspondence across individuals
means that even high-performing anatomical alignment does not ensure
fine-grained functional alignment [e.g., \citet{frost2012measuring}]''
...
%
``More sophisticated strategies for registering
...
temporal lobe [\citep{spiridon2006location}].
%
State-of-the-art cortical surface-based alignment [\citep{fischl2012freesurfer}]
that respects sulcal locations can reduce but not eliminate the mismatch
...
\todo[inline]{following should be more way shorter}
% intro
...
time series \citep{haxby2011common, guntupalli2016model, chen2015reduced,
zhang2016searchlight} or connectivity-patterns
\citep{guntupalli2018computational, nastase2019leveraging}, based on their
similarity to a high-dimensional \textit{common functional space}.

%
Different subjects are aligned based on the maximization of the
inter-subject similarity of \ac{bold} responses \citep{haxby2011common,
chen2015reduced, sabuncu2010function} correlating with a time-locked external
stimulation (e.g. movie or auditory narrative), or connectivity profiles
\citep{feilong2018reliable, guntupalli2018computational, nastase2019leveraging}.

%
``Naturalistic stimuli such as movies and stories sampling a broader range of
brain states than paradigms using simplified stimuli are often used to generate
such training data. Nevertheless, any study design in which participants perform
the same sequence of trials could be used  (for example, a battery of cognitive
tasks)'' [still kinda phrasing from] \citep{cohen2017computational}.
...
\paragraph{studies that estimated functional areas}
...
In the current study, we employ the \ac{srm} \citep{chen2015reduced,
...
%
The SRM approach warps individual representational geometry into common
...
\citep{kumar2020brainiak}.\todo{what? ;-)}
...
\todo[inline]{merge into text above; used compiled/printed PDF!}
...
%
We compare the prediction performance of a volume-based, anatomical alignment
procedure to a volume-based, functional alignment procedure.
...
in the visual localizer paradigm \citep{sengupta2016extension} but
also watching a Hollywood \citep{hanke2016simultaneous} movie and listening to
the movie's audio-description \citep{hanke2014audiomovie}.

\todo[inline]{make same order of steps + phrasing as in explanation above}

%
Results of our previous study suggest that naturalistic stimuli sample the
response vector space of "spatial responses" time-locked across subjects.
