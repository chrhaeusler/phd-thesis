...
or \ac{eba} \citep{downing2001bodyarea} exhibit significantly increased
\ac{bold} activity correlated with a ``preferred'' \citep[][p.
...
However, conducting a two-hour long fMRI scan session may not be desirable or
...
brain anatomy to provide an estimate a functional region's location.
...
subject-specific transformations from the functional data of a study's
participants.
%
...
\todo[inline]{phrase general question}

\todo[inline]{drop validity / generalizability thing?}
...
functionally a line a subject to the \ac{cfs} by projecting hemodynamic
...
\textbf{The purpose of this study was to investigate four aspects}.
%
First, we explored the \textbf{validity and generalizability} of our
multi-paradigm \ac{cfs} by predicting a left-out \textit{test subject}'s results
...
%
This \textit{partial alignment} lets us assess the \textbf{validity and
generalizability} of the three paradigms serving as \textit{predictors} (i.e.
one \textit{cross-subject-within-paradigm prediction}, and two
...
in a clinical\todo{drop it} setting to align a test subject, we also explored
the relationship between the estimation performance of the results from each of
the three paradigms and the quantity of data from each of the three paradigms
used to functionally align the subject with the multi-paradigm \ac{cfs}.
...
localizer and naturalistic stimuli.
%
...
localizer and naturalistic stimuli separately for each paradigm and training
subject. We then concatenated the time series, fit the \ac{srm}, and calculated
...
decomposition.
%
This step produced a transformation matrix $W_{n}$ with orthonormal
columns for each paradigm
...
%
Specifically, we we used one up to four runs (each lasting \unit[5.2]{min}) of
the visual localizer, and one up to eight runs (each lasting
...
%
Next, we used the transpose of a transformation matrix obtained from
the alignment of the test subject to project the $Z$-maps from the \ac{cfs} into
the test subject's voxel space.
% take the mean
...
We then used the \textbf{transpose} of the transformation matrix to project the
data from the MNI space into the test subject's voxel space.
% take the mean
...
\todo[inline]{Discussion of poor performance of localizer is obsolete}
...
validity / generalizability of transformations}
...
$\approx$$\approx$\unit[15]{min}utes (\ac{tr}=2s) of movie watching used for
...
pattern estimated from a healthy or clinical\todo{drop?} reference group.
