...
subject-specific transformations based on functional data of study participants.
%
...
%
While connectivity-based functional alignment is more effective in aligning
connectivity profiles, response-based functional alignment is more effective in
aligning response time-series \citep{guntupalli2018computational}.
%
...
$Z$-maps) serving as the \textit{criteria} to be predicted.
%
The $t$-contrast were created in previous studies that aimed at identifying the
...
%
the movie ``Forrest Gump'' ($\approx$\unit[120]{min}, split into 8 runs;
\ac{tr}=\unit[2]{s}),
%
the movie's audio-description that was produced for a visually impaired audience
($\approx$\unit[120]{min}, split into 4 runs; \ac{tr}=\unit[2]{s}), and
%
the visual localizer ($\approx$\unit[20]{min} split into 4 runs;
\ac{tr}=\unit[2]{s})
%
were concatenated and fed into the \ac{srm} algorithm in order to calculate the
\ac{cfs} and the training subjects' transformations (see
Fig.~\ref{fig:multi-stimulus-cfs}).
% test subject's transformation
In order to be able to investigate the prediction performance of each paradigm,
we then performed a \textit{partial alignment} of the \textit{test subject}:
%
...
of feasible, we also investigated the relationship between the quantity of data
...
% VIS
(a) participated in a six-category block-design visual localizer
...
\acp{tr} from the visual localizer (7747 \acp{tr} in total).
...
\todo[inline]{Make this part shorter (streamline with introduction)}

\todo[inline]{Stuff explained but deleted here must be in the parts below}
%
...
calculating the arithmetic mean of the projected empirical $Z$-maps.
...
stimuli
%
to align the test subject's time series with the their corresponding \acp{tr}
within the \ac{cfs}.
%
...
subject-specific masks that was applied to the time series data used to generate
the \ac{cfs}.
%
...
%
using the training subjects' results of the same paradigm (hence, three
...
\todo{empty}
%
We calculated the Pearson correlation coefficients between each individual's
empirical $Z$-maps obtained from previous analyses
\citep{haeusler2022processing, sengupta2016extension} and their respective
predicted $Z$-maps to assess the performance of the alignment procedures (see
...
the within-paradigm prediction based on an anatomical alignment
...
correlations between empirical $Z$-maps and predicted $Z$-maps via
the within-paradigm prediction based on an anatomical alignment ($t(14)= 5.78$, $p<.0001$).
%
...
\todo[inline]{streamline with intro}

\subsubsection{Problem space}

\todo[inline]{localizers are tedious}

%
To overcome the limitations associated with lengthy scanning procedures, an
alternative approach can be employed.
%
This approach involves localizing a functional area in an individual by
utilizing data collected from an independent sample of other individuals, often
referred to as a reference group. By leveraging the data from this reference
...
\paragraph{So far: anatomical alignment}


\subsubsection{We: functional alignment}

\todo[inline]{could a movie used to localize? do we need 120 min?}
%
...
\subsubsection{Results (if at all, very short)}
%
...
\todo[inline]{it is actually not that bad; but it's always within-paradigm
prediction!}
...
15 min of movie with partial alignment outperformed anatomical alignment.
%
...
%
However, using just 15 min movie resulted in worse performance than
anatomical alignment.
...
%
However, it improves when more data are used for alignment.
%
...
anatomical alignment when more than 4-6 runs (a.k.a. minutes) are used.
%
...
\todo[inline]{focus on estimation of localizer (because "gold standard")}

\todo[inline]{generally, movie is best; which is in line with previous research}


\subsubsection{Visual localizer for functional alignment}

%
Is best for within-paradigm prediction;
%
also works well if movie PPA is predicted;
%
results are in line with \citet{haxby2011common}

\subsubsection{Movie for functional alignment}

%
When estimating the visual localizer, results indicate that
$\approx$\unit[15]{min}utes (\ac{tr}=2s) of movie watching used for
functional alignment outperform prediction using anatomical alignment.
%
Prediction performance further increases when
$\approx$$\approx$\unit[30]{min}utes of movie data submitted to the algorithm to
...
scanning time than $\approx$\unit[30]{min}utes during audio-visual naturalistic
stimulation.


\subsubsection{Audio-description for functional alignment}

\todo[inline]{cross-modal estimation}
