...
\todo[inline]{check \citep{jiahui2022cross, feilong2022individualized}}
...
\todo[inline]{Is cross-paradigm and within-paradigm estimation / prediction
defined in the intro? These terms make discussion more concise to write}
...
\todo[inline]{It's kind of difficult (if not even wrong) to write "umbrella
hypotheses"; in any case, I was expecting the localizer runs to better predict
the localizer results (cf. \citet{haxby2011common}}
...
\todo[inline]{Plot of "number of voxels" is added; table with exact numbers is
removed}
...
\centering
    ...
    \caption{
    %
        \textbf{Number of voxels in the subject-specific regions of interest
        (ROIs).}
    %
    Each subject's time series and $Z$-maps were masked in their subject's
    voxel space by
    the union of individual \acp{ppa} \citep[cf.][]{haeusler2022processing}
    that was warped from MNI152 space into each subject's voxel space
    and the subject-specific \ac{fov} of the audio-description.
    }
...
\todo[inline]{I shifted this up in front (now more in line with the intro); it
primes the (similar but more complicated) functional alignment, too}
...
\todo[inline]{I created the "other-dimensional" \acp{cfs} \& printed the
Pearon's r's to file; judged by eye balling results were not "better"}
...
cf. Table 3 in \citet{haeusler2022processing}}

\todo[inline]{What is the point to make with presented plots as support?}

\todo[inline]{not presented: a plot of (just the) shared features but across all
TRs within the \ac{cfs}; "problem" in the plots shown below: the correlations
between the shared features vary depending on the TRs used => there is no clear
"face" or "building" shared feature across paradigms.}
...
\todo[inline]{I have plots prepared for sub-05 (the biggest outlier regarding
Cronbach's $\alpha$ of audio PPA): \ac{cfs} calculated from all subjects' data
except sub-05 looks different; of course different value but also (slightly?)
...
\todo[inline]{Give the median / mean Cronbach's here (already although it's
strictly results}

\todo[inline]{btw, sub-05 is the asshole: Cronbach's of -0.5}
...
\todo[inline]{make list of which statistical tests to perform!}

\todo[inline]{many cases should require non-parametric test; especially in case
of auditory PPA as criterion and audio-description as predictor (Fisher's
Z-transform should not change that, probably)}

\todo[inline]{adjust 'statistics\_t-test-correlations.py' accordingly}

\todo[inline]{the plot: add median Cronbach's to the plot; take same line style
as in the plot plot of Cronbach's per subject}
...
\begin{figure*}[tbp] \centering
    ...
    \textit{\textbf{Z}}-maps for each subject.}
    %
    Dashed lines:
    %
    median of Cronbach's $\alpha$ across subjects for the respective paradigm's
    $t$-contrast (i.e. Cronbach's $\alpha$ of empirical $Z$-maps).
    %
    Grey dots:
    %
    ...
    segments (each lasting $\sim$\unit[15]{m}) of the movie.
    %
    Blue dots:
    %
    correlations between empirical $Z$-map and an estimation using functional
    alignment based an transformation matrices calculated from one up to eight
    segments (each lasting $\sim$\unit[15]{m}) of the audio-description.
    %
    \textbf{to do: keep underlying boxplots (including mean \& median)?? Add
    $Z$-maps median Cronbach's alpha? to legend?)}
}
...
\subsection{Template from \citet{jiahui2020predicting}}
...
\todo[inline]{cf. "Templates for Cronbach's" at the very end of the chapter}

\todo[inline]{print, read \& understand it, or die trying}
...
\todo[inline]{cf. paper that used Cronbach's and put some templates here}
...
\todo[inline]{The stripplot is a mess; but almost always the same 3 outliers.}
...
\todo[inline]{The outliers are sub-02, sub-04, sub-05 for 1-2 runs; with 3-4
runs sub-04 gets comparatively better, and it's sub-20 again}
...
\todo[inline]{The outliers are always sub-05, sub-02, sub-20; who have the
lowest values of Cronbach's alpha, too}
...
\subsubsection{Functional vs. anatomical alignment (it's not that bad actually)}

Localizer paradigm sucks compared to anatomical alignment when used to estimate
localizer (within-paradigm prediction) and movie PPA.
%
Localizer for alignment gets close to anatomical alignment when audio PPA is
estimated.
...
estimation (it gets better the more data you use, though).
%
...
anatomical alignment (when use more than 4-6 segments)
%
...
alignment, too.
%
Maybe, the outliers fucked up the model space?
...
\todo[inline]{extend the idea to lots of "localizers" below in "Vision"!!! which
should be shifted mostly to the general discussion}
...
and results generalize to across-paradigm prediction (e.g. audio-description for
alignment to estimate $Z$-maps from the visual localizer's $t$-contrast).

%
The latter case is different from findings of \citet{haxby2011common} who found
that within-paradigm prediction of $Z$-maps from a non-naturalistic paradigm
stimulus via non-naturalistic paradigm was as good or [even] better than
cross-paradigm prediction via naturalistic stimulus for alignment.
%
But: they had a \ac{cfs} that was just derived from the non-naturalistic
stimulus paradigm, i.e. no multi-stimulus paradigm.
%
Maybe, our naturalistic stimuli (about 7000 TRs) fucked up our shared responses
in 450 TRs within the SRM (imo, Fig.~\ref{fig:corr-vis-reg-srm} suggests
otherwise)
...
%
The auditory response is (too) different from the visual response?
%
Audio-description gave more room for individual variation of responses?
%
Some subjects where just not attending or even drove off to sleep?

%
We need---of course---further studies to assess the reliability of individual
differences in responses to auditory stimulation.
%
...
%
Similar to the standard procedure of a anatomical scan, to perform a functional
alignment to a functional atlas.
%
...
calibration scan could align a subject to a \ac{cfs} derived from
extensive scans and analyses of other subjects' data.
%
The \ac{cfs} serves as an \textit{functional atlas}.

\todo[inline]{define functional atlas / database here}
...
\paragraph{Naturalistic stimulus: validity / generalizability of matrices}

\todo[inline]{Merge following two paragraphs to "two advantages"}
...
by a movie or narrative (language, memory, emotions etc.)}
...
\paragraph{Naturalistic stimuli: compliance, visually impaired}
%
...
(but prime the subject the stimulus features within the naturalistic
stimulus).


\paragraph{Simplified stimuli: for signal detection, "gold standard"}

The database would also provide data from additional paradigms that are
dedicated to investigate specific brain responses
%
because modeling of responses to specific stimulus features is fucking difficult
and the dedicated paradigms are, well, designed / dedicated to probe specific
brain process (and are the "gold standard").
...
%
Functional patterns in an individual could be inferred from a database created
...
\paragraph{Application: Quantify deviation from regular pattern}

\todo[inline]{cf. shortcoming of SRM: estimation in case of atypical pattern}
...
\todo[inline]{Condense the following to the gist of the studies: the
functional-anatomical correspondence differs across ROIs}
...
\todo[inline]{Were to put the following?}
%
...
\todo[inline]{check \citep{feilong2022individualized, jiahui2022cross,
turek2018capturing} for modelling an individual component}
