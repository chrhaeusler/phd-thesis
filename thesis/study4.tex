...
smoothed (Gaussian kernel with full width at half maximum of \unit[4.0]{mm}).  A
grand-mean intensity normalization was applied to each run of the functional
...
model.
% union of PPA masks
Hence, in order to create bilateral \acp{roi}, we warped the union of individual
\acp{ppa} \citep[s.][]{haeusler2022processing} [via non-linear transformation x
degrees of freedom] from MNI space into each subjects' voxel space, and applied
it as a mask to each subjects' time series data.
% AO FoV
...
% explain shit
During model fitting, the algorithm used each $n^{th}$ training subject's
response time series represented as matrix $X_{n}$ ({$v$} voxel by $t$ time
points) to calculate the \ac{cfs} $C$ ($k$ features by $t$ time points) and
subject-specific, orthogonal transformation matrices $T_{n}$ ($v$ voxel by $k$
features).
...
responses. Each transformation matrix allows to project the responses of voxels
within a subjects' bilateral \acp{roi} from anatomical voxel space into the
$k$-dimensional \ac{cfs}, and thus functionally aligns a subject data.
% iteratively fitted
...
\todo[inline]{imo, the correlation matrices of regressors and shared responses
should be brought up here already and not in the results; as it is done in the
PPA paper}
...
% shuffled data
As negative control, we shuffled the order of runs of the movie and
audio-description [but not yet of visual localizer!] before fitting a shared
response model to the time series in order to assess the correlations (Pearson's
r) between the regressors modeled in the previous studies and the shared
responses in the corresponding \acp{tr}. Results revealed no [to minor; check to
give numbers] correlations between the regressors and shared responses.
