...
\subsubsection{Discussion \& open questions}
...
%
``The introduction of dynamic videos of faces and control categories to
...
\todo[inline]{why did we use SRM in the first place? Mention some sentences in
the intro, too:
%
a) nicely implemented in BrainIAK (cf. general intro on reproducibility);
%
b) we do not use the fastSRM algo, but it's probably still computationally less
demanding than hyperalignment; ``computational efficiency is an important
consideration for scientists who may not have access to specialized hardware''
(cf. also evaluation of algos in \citet{chen2015reduced,
bazeille2021empirical})}

\todo[inline]{cf. \citet{bazeille2021template}'s dissertation}


\paragraph{Volume- vs surface-based}
...
are tightly bound to anatomical landmarks, i.e. there is no general rule
describing how macroanatomical and functional areas correlate''
\citep{frost2012measuring}.
%
