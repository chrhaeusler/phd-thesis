...
\todo[inline]{I shifted this up in front (now more in line with the intro);
it kind of primes the (similar but more complicated) functional alignment, too}
...
\todo[inline]{Problem for (parametric) t-tests: Correlations (especially in case
of auditory PPA) are not normally distributed (Fisher's Z-transform should not
change that, probably)}

\todo[inline]{'statistics\_t-test-correlations.py'}
...
\todo[inline]{This should be rather short, cause there is a "Discussion across
localizer PPA, movie PPA \& audio PPA" later.}
...
One segment of movie beats [test that] anatomical alignment?
%
...
\todo[inline]{wow, that stripplot is a mess...; but hey, pretty much always the
same three outliers; which are probably the outliers in Cronbach's alpha of the
audio PPA z-map.}
...
\todo[inline]{parametric statistical test are "difficult" because non-normal
distribution}
...
\todo[inline]{my biggest issue: what to do with the three outliers?}
...
\todo[inline]{well, if you cannot predict the $Z$-map "correctly" you could at
least quantify how much the empirical $Z$-map is different from the prediction
...
\todo[inline]{does one single paper except \citet{haxby2011common} exist that
derives a \ac{cfs} from a non-naturalistic stimulus?}

\todo[inline]{However, nothing comes close to the reliability (Cronbach's
$\alpha$) of the localizer $Z$-maps.}
...
should be shifted to the general discussion}


\paragraph{Introductory statement}
%
Our results provide evidence that transformation matrices calculated based on
data from naturalistic stimuli promise an increased validity for functional
alignment over transformation matrices based on data of the same [!] paradigm
based on simplified stimuli (kind of different to \citet{haxby2011common}).
...
Moreover, our results suggest that just a part of a naturalistic stimulus
provides enough data to estimate the $Z$-maps.
%
It is not "the more, the better", no linear but asymptopic relationship
%

This provides the oppportunity to add "diagnostic" (comparably to the standard
anatomical scan session) during an fMRI experiment.
%
Moreover, it is easier to share a stimulus across experiments or research groups
than participants \citep[e.g.][]{zhang2018transfer}.
...
In summary, compared to paradigms with simplified stimuli, naturalistic stimuli
sample a broad range of brain states \citep{guntupalli2016model,
haxby2011common} that reflect (confound) statistics of the natural environment,
promising an increased validity of transformations of functional alignment, and
might enable investigation of the acquired data for a variety of research
questions to research questions/domains/paradigms (e.g. visual or auditory
perception, spatial cognition; emotion; music, speech or social perception).
...
%
This could serve as an estimation of a persons functional pattern and/or
allows a comparisons to a persons "actual pattern" to a normative reference
...
\paragraph{Applied to our results: some templates}
