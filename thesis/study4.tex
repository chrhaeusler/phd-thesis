...
\acp{cfs}, one \ac{cfs} based on data of a controlled paradigm using a smaller
stimulus set of still images [limited number of stimulus categories] and another
\ac{cfs} based on movie data (of comparable length).
%
Results revealed a higher cross-subject classification of the traditional
paradigm's stimulus categories via a \ac{cfs} and transformations calculated
based on data of the same paradigm compared to a cross-subject classification of
the stimuli via the \ac{cfs} and transformations calculated based on data
%
``Result suggests that the validity for a model of a specific subspace may be
enhanced by designing a stimulus paradigm that \textbf{samples the brain states
in that subspace more extensively}'' \citep{haxby2011common}.
%
...
\todo[inline]{maybe, add inference: generalization from estimating $Z$-maps of a
traditional paradigm to $Z$-maps from a naturalistic paradigm.}
...
%
\textbf{First}, it is possible that the audio-description ``not as rich'' [less
diverse variety of representational states] as
the movie and does not sample the response vector space as comprehensively,
which becomes more pronounced when estimating the results of visual paradigms
due to the lack of visual stimulation during the auditory paradigm.

%
\textbf{A second}, not mutually exclusive, explanation might be inferred from
the fact that the amplitudes of the shared responses during the \acp{tr} of the
...
response in the \ac{ppa} to an auditory paradigm differs from a response to a
...
\todo[inline]{\citet{haxby2011common} kind of contradicts the
"too-different-response-hypothesis"}

\todo[inline]{however: maybe, the maps are "amazingly well" estimated in the
anterior part but "badly" estimated in the posterior part?}

\todo[inline]{probably, it's more an issues of number of samples per brain
state!}

\citet{haxby2011common}: ``We also tested whether the general validity of the
model space reflects responses to stimuli that are in both the movie and the
category perception experiments or reflects stimulus properties that are not
specific to these stimuli.
%
...
%
``Moreover, the general validity of the model based on the responses to the
movie is not dependent on responses to stimuli that are in both the movie and
the category perception experiments but, rather, appears to rest on stimulus
...
Cronbach's $\alpha$ show also showed the lowest correlation between predicted
...
%
However, disentangling assessing the degree to which the modeled \ac{cfs}, the
transformation matrices of a test subject, and the mean reliability of the
$Z$-maps influence the estimation performance is challenging in the current
study.
%
Given that the within-paradigm prediction works "well" for most subjects given
sufficient amount of data to align the subject, the time series of the outliers
do not significantly diminish the (validity of the) model and suggest that the
\ac{srm} is robust against outliers.
%
...
exclusively auditory experiment, such as a speech localizer, that yield more
reliable results in all subjects.
%
Such a study could then also compare the performance of an auditory narrative to
the performance of an audio-visual movie to predict the results of a exclusively
auditory paradigm.
%
In order to test the hypothesis that the \ac{srm} filters out noise, a more
reliable criterion could be estimated by aligning a test subject and estimate
second-level $Z$-maps from actual time-series and second-level $Z$-maps
calculated from time-series with added artificial noise at different levels of
intensity.
%
\todo[inline]{if you know what I mean: take the noisy time series for alignment;
of test subject; does procedure better predict the $Z$-maps calculated from
actual time-series or $Z$-maps calculated from noisy time-series?}
...
our test (estimating $Z$-maps not BSC of movie segments) easier?}
