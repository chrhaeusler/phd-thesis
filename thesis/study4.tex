...
\todo[inline]{movie \& audio PPA are bonus}

\todo[inline]{exact mean (z-transformed) correlation per paradigm for anatomical
alignment?}
...
%
Overall, results (cf. Fig.~\ref{fig:stripplot}) that the estimation based on
functional alignment approaches a performance limit and a decreasing benefit of
...
%
The prediction based on data of the audio-description revealed a lower
performance compared to the anatomical alignment, and the functional alignment
based on the localizer data or the movie data.
%
...
%
Results suggest that an auditory paradigm could, in principle, be substitute a
...
\todo[inline]{cf. time series plot of shared responses: weaker (or just smeared)
responses across subject during the auditory paradigm}

\todo[inline]{the ROI is biased to favor results of the visual localizer since
it is the union of individual \acp{ppa} and therefore favors voxels that are
significantly increased activity (tacit assumption is that localizer, movie,
audio-description lead to increased activity in the same set voxels)}
...
alternative way to localize the \ac{ppa}, the mean prediction performance of the
anatomical alignment is similar to the prediction performance when estimating
the dedicated localizer paradigm.\todo{why is that? numbers?}

%
Similar to the estimation of the visual localizer's results, the within-paradigm
predictions lead to the highest correlations between empirical and predicted
$Z$-maps.
%
Moreover, as in the visual localizer paradigm, estimating the results of the
movie via the audio-description revealed the lowest estimation performance of
all alignment approaches.

%
Funnily, the within-paradigm prediction of the 15 min audio-description to
predict the $Z$-maps from the audio-description is as good as / not better than
an prediction via anatomical alignment.
%
This is different from the other two within-paradigm prediction via the visual
paradigms.

%
The more data are used the better it gets.
%
When estimating visual paradigms (cross-paradigm) via audio-description the gap
to estimation via movie gets more narrow.
%
When estimating the audio PPA (within-paradigm) the mean correlation gets
better, higher than anatomical alignment and the Cronbach's a of the $Z$-maps
(not tested statistically but judge by visual inspection).
%
\textbf{Inferences:} This suggests the in principle usability of an exclusively
auditory paradigm to estimate the results of a visual paradigm, however at the
expanse of an extensive data collection.
%
Moreover, results suggest that \ac{srm} might denoise data from outliers
\todo{carefully, bro!}



\paragraph{Reasons why audio-description might suck}
%
A feasible reasons might be that the audio-description is ``simply not as rich''
as the movie given that it lacks a visual simulation and therefore does not
sample the response vector space as much as the movie.
%
Another, not mutually exclusive explanation might be that the response in the
\ac{ppa} to an auditory paradigm is (too) different from a response in the
\ac{ppa} to a visual paradigm.\todo{cf. \citet{haeusler2022processing}}
%
A third, again not mutually exclusive, reason might be that the stimulation
during the audio-description is correlated with weaker responses in the
investigated \ac{roi}.
...
%
\textbf{Inference:} limited suitability to substitute a visual paradigm; an
obvious question is to do the vice versa and estimate auditory process /
paradigm via narrative and movie.
...
In summary, our results provide evidence that $\approx$\unit[15]{min} of movie
...
longer scanning time.
%
Audio-description has comparatively low cross- and within-paradigm prediction
when merely $\approx$\unit[15]{min} minutes are used to align a test subject.

%
We have shown that it is in principle possible to use an auditory
paradigm to predict a visual paradigm (but does not really make sense?)
%
...
\subsubsection{Problems with naturalistic stimuli as localizer / criterion}
