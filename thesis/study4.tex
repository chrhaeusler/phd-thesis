...
\todo[inline]{I restructured the discussion according to "three aspects under
investigation" (at least, I started to do it; it's not final)}
...
\todo[inline]{mention "within-paradigm" and "cross-paradigm" prediction}

%
We created a \ac{cfs} (for every fold) based on time-series of three stimuli
(visual localizer, movie, movie's audio-description) that were concatenated
before model fitting.
%
For every paradigms $Z$-map (visual localizer, movie, movie's audio-description)
serving as criteria to be estimated, we use a test subject's response time
series to the visual localizer, movie, or the audio-description independently as
...
\todo[inline]{To be written; the high-level summary of results here, heavily
depends on how the discussion turns out to be}
...
in audio PPA are difficult to estimate.


\paragraph{Conclusion anatomical vs. functional alignment}

Anatomical alignment is actually not that bad.
%
It pretty consistent across paradigms, too.
...
for alignment so "bad"? Is it our model or the transformation matrices? In any
case, our results are different from \citep{haxby2011common}: \acf{cfs} \&
transformation matrices based on visual localizer that predict the localizer as
good as / better than than \ac{cfs} \& transformation matrices based on movie!}
...
\paragraph{Why is localizer to predict localizer so bad?}
...
Surprisingly, prediction of localizer via localizer runs (i.e. within-experiment prediction is worse than anatomical alignment.
%
How does that come? At least the correlations of regressors with
shared responses show the "clearest" correlations of a regressor with a shared
response (which is not the case in the \acp{tr} of the movie or
audio-description).
...
\paragraph{Generalizability of naturalistic stimuli (esp. movie)}

\todo[inline]{We have way more \ac{tr} in case of naturalistic stimuli; however
...
\todo[inline]{summarize to its gist}
...
\todo[inline]{problem: \citet{feilong2022individualized} assess data quantity,
too; results suggest 30m are "good" in case of their model; plus, they model
individual component.}


\paragraph{Estimation of localizer (a.k.a. "gold standard")}
...
\todo[inline]{That's essentially why we estimated non-localizer $Z$-maps, too;
which is not explicitly stated in the intro; I did not know where to state it
there; imo, it's more interesting to discuss in the general discussion}
...
\todo[inline]{following should also be true for movie PPA but it's more severe
in case of audio-description}

\todo[inline]{the "deviant" participants in the audio-description are
essentially the ones that have a poor Cronbach's; so, they are less probably not
"reliable outliers" being different from the norm but more probably noisy asses;
hence, SRM should probably do more of a denoising as opposed to modeling
(reliable) outliers incorrectly}

\todo[inline]{also a topic for the general discussion: reliability of an
auditory naturalistic stimulus as a localizer; what's wrong?  the modeling in
\citet{haeusler2022processing}? Or does an auditory naturalistic stimulus give
too much room to participants to do not give a shit? a.k.a. freely listening is
bad?}
