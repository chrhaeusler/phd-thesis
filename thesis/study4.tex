...
Consequently, each shared feature can be thought of as a weighted sum of many
...
($\approx$\unit[120]{min}; \ac{tr}=\unit[2]{s}), and
%
the visual localizer ($\approx$\unit[20]{min}; \ac{tr}=\unit[2]{s})
%
...
%
a test subject's response time series from each of the three paradigms were
separately aligned with the corresponding \acp{tr} within the \ac{cfs} to obtain
...
\todo[inline]{localizers are tedious; could a movie used to infer localization;
do we need 120 min?}
