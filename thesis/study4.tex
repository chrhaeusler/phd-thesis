...
time; at the moment, I did not see any got spot in the intro; but should be
primed for the discussion in the intro, though.}
...
previous studies; was there any cfs based on paradigm x, alignment via paradigm
y???}
...
\centering
    ...
    to calculate the \ac{cfs} $C$ ($k$ shared features by $t$ time points)
...
Questions as discussed during our meeting:
\begin{itemize}
...
the localizer results (cf. hypotheses)}
...
\todo[inline]{goddamnit, this is actually the first step of the functional
...
%
We assumed to Cronbach's $\alpha$ to be a noisei ceiling, and ``we reasoned that
...
\todo[inline]{Problem for t-tests: Correlations (especially in case of auditory
PPA) are not normally distributed (Fisher's Z-transform should not change that,
probably)}
...
%
Validity should be the cross-subject-\textbf{within-paradigm} prediction.
%
Generalizability should be the cross-subject-\textbf{cross-paradigm} prediction.
...
\paragraph{We: alignment via three paradigms}
...
\paragraph{We: partial alignment}
...
\todo[inline]{estimated using different predictors: anatomically \& functionally
based on three paradigms (localizer, movie, audio-description)}

\todo[inline]{How good is which stimulus to perform alignment to that
(sub)model?}


\subsubsection{Assumption that localizer is "ground truth"}

\todo[inline]{This might be shifted downwards as intro to "prediction of movie
and audio PPA}
...
\subsubsection{visual localizer via anatomical alignment}
...
\todo[inline]{check papers; they should all use whole stimulus matrices; also
check for validity of \ac{cfs} based on different paradigms}
...
\subsubsection{Haxby et al. on validity and generalizability}
...
\subsubsection{movie PPA via anatomical alignment}
